...
bool8 GetCanKnockOut(u8 bankAtk, u8 bankDef)
{
	...
	if (IsBankIncapacitated(bankAtk) || gAbsentBattlerFlags & (gBitTable[bankAtk] | gBitTable[bankDef]))
		return FALSE;
...
bool8 MoveKnocksOutPossiblyGoesFirstWithBestAccuracy(u16 move, u8 bankAtk, u8 bankDef, bool8 checkGoingFirst)
{
	...
	for (int i = 0; i < MAX_MON_MOVES; ++i)
	{
		...
		if (!(gBitTable[i] & moveLimitations))
		{
			...
			if ((!checkGoingFirst || MoveWouldHitFirst(currMove, bankAtk, bankDef)) && MoveKnocksOutXHits(currMove, bankAtk, bankDef, 1))
			{
...
bool8 IsWeakestContactMoveWithBestAccuracy(u16 move, u8 bankAtk, u8 bankDef)
{
	...
	for (int i = 0; i < MAX_MON_MOVES; ++i)
	{
		...
		if (!(gBitTable[i] & moveLimitations) && CheckContact(currMove, bankAtk) && moveEffect != EFFECT_RECHARGE && moveEffect != EFFECT_COUNTER && moveEffect != EFFECT_MIRROR_COAT && moveEffect != EFFECT_BURN_UP && moveEffect != EFFECT_FAKE_OUT && moveEffect != EFFECT_0HKO) // Don't use these move effects on partner
		{
...
bool8 StrongestMoveGoesFirst(u16 move, u8 bankAtk, u8 bankDef)
{
	...
	for (int i = 0; i < MAX_MON_MOVES; ++i)
	{
		...
		if (!(gBitTable[i] & moveLimitations))
		{
			...
			if (MoveWouldHitFirst(currMove, bankAtk, bankDef) && currDmg > bestDmg)
			{
				bestDmg = currDmg;
				bestMoveIndex = i;
			}
...
#undef CALC
			...
			if (gBattleMoves[move].target & MOVE_TARGET_ALL && RangeMoveCanHurtPartner(move, bankAtk, bankAtkPartner))
			{
				u8 status1 = gBattleMons[bankAtkPartner].status1;
				if ((status1 & STATUS1_SLEEP) <= 1 // Partner will be awake to use move
					&& !(status1 & STATUS1_FREEZE) && !gDisableStructs[bankAtkPartner].truantCounter)
				{
					if (partnerMove != MOVE_NONE) // Partner has chosen a move
					{
						u8 moveEffect = gBattleMoves[partnerMove].effect;

						if (moveEffect == EFFECT_PROTECT && partnerMove != MOVE_QUICKGUARD && partnerMove != MOVE_CRAFTYSHIELD && (partnerMove != MOVE_MATBLOCK || gDisableStructs[bankAtkPartner].isFirstTurn))
						{
	...
	{
		u8 currScore = moveScores[i][bankDef] + moveScores[i][bankDefPartner] + moveScores[i][bankAtkPartner];

		u8 bestScore = moveScores[bestIndex][bankDef] + moveScores[bestIndex][bankDefPartner] + moveScores[bestIndex][bankAtkPartner];
		...
		else if (currScore == bestScore)
		{
			...
			if (currentBestRange & MOVE_TARGET_ALL && !(checkBestRange & MOVE_TARGET_ALL) && checkBestRange & MOVE_TARGET_BOTH && RangeMoveCanHurtPartner(gBattleMons[bankAtk].moves[bestIndex], bankAtk, bankAtkPartner))
...
{
	if (gBattleMoves[move].target & MOVE_TARGET_ALL && gBattleMoves[gChosenMovesByBanks[PARTNER(bankAtk)]].effect == EFFECT_PROTECT && !gNewBS->recalculatedBestDoublesKillingScores[bankAtk])
	{
	...
	if (move == gNewBS->ai.bestDoublesKillingMoves[bankAtk][bankDef])
	{
		...
		return gNewBS->ai.bestDoublesKillingScores[bankAtk][bankDef][bankDef] + gNewBS->ai.bestDoublesKillingScores[bankAtk][bankDef][PARTNER(bankDef)] + gNewBS->ai.bestDoublesKillingScores[bankAtk][bankDef][PARTNER(bankAtk)];
	}
...
{
	return BATTLER_ALIVE(bankAtkPartner) && ABILITY(bankAtkPartner) != ABILITY_TELEPATHY && !(BATTLER_SEMI_INVULNERABLE(bankAtkPartner) && MoveWouldHitFirst(move, bankAtk, bankAtkPartner)) // Target will still be semi-invulnerable when attack hits
...
move_t CalcStrongestMove(const u8 bankAtk, const u8 bankDef, const bool8 onlySpreadMoves)
{
	...
	for (u32 i = 0; i < MAX_MON_MOVES; ++i)
	{
		...
		if (!(gBitTable[i] & moveLimitations))
		{
			if (gBattleMoves[move].power == 0 || (onlySpreadMoves && !(gBattleMoves[move].target & (MOVE_TARGET_BOTH | MOVE_TARGET_ALL))))
				continue;
			...
			else
			{
				...
				else if (predictedDamage == highestDamage && PriorityCalc(bankAtk, ACTION_USE_MOVE, move) > PriorityCalc(bankAtk, ACTION_USE_MOVE, strongestMove)) // Use faster of two strongest moves
				{
					strongestMove = move;
				}
...
bool8 MoveWillHit(u16 move, u8 bankAtk, u8 bankDef)
{
	...
	if (ABILITY(bankAtk) == ABILITY_NOGUARD || defAbility == ABILITY_NOGUARD || (gStatuses3[bankDef] & STATUS3_ALWAYS_HITS && gDisableStructs[bankDef].bankWithSureHit == bankAtk))
		return TRUE;

	if (((gStatuses3[bankDef] & (STATUS3_IN_AIR | STATUS3_SKY_DROP_ATTACKER | STATUS3_SKY_DROP_TARGET)) && !CheckTableForMove(move, gIgnoreInAirMoves)) || ((gStatuses3[bankDef] & STATUS3_UNDERGROUND) && !CheckTableForMove(move, gIgnoreUndergoundMoves)) || ((gStatuses3[bankDef] & STATUS3_UNDERWATER) && !CheckTableForMove(move, gIgnoreUnderwaterMoves)) || (gStatuses3[bankDef] & STATUS3_DISAPPEARED))
		return FALSE;

	if ((move == MOVE_TOXIC && IsOfType(bankAtk, TYPE_POISON)) || (CheckTableForMove(move, gAlwaysHitWhenMinimizedMoves) && gStatuses3[bankDef] & STATUS3_MINIMIZED) || ((gStatuses3[bankDef] & STATUS3_TELEKINESIS) && gBattleMoves[move].effect != EFFECT_0HKO) || gBattleMoves[move].accuracy == 0 || (WEATHER_HAS_EFFECT && (gBattleWeather & WEATHER_RAIN_ANY) && CheckTableForMove(move, gAlwaysHitInRainMoves)) || IsZMove(move) || IsAnyMaxMove(move))
		return TRUE;
...
bool8 IsUsefulToFlinchTarget(u8 bankDef)
{
	...
	if (IsBankAsleep(bankDef) && !MoveInMoveset(MOVE_SLEEPTALK, bankDef) && !MoveInMoveset(MOVE_SNORE, bankDef))
		return FALSE;
...
bool8 IsBankIncapacitated(u8 bank)
{
	...
	if (gBattleMons[bank].status2 & STATUS2_RECHARGE || (ABILITY(bank) == ABILITY_TRUANT && gDisableStructs[bank].truantCounter != 0))
		return TRUE;
...
bool8 WillTakeSignificantDamageFromEntryHazards(u8 bank, u8 healthFraction)
{
	...
	if (gSideStatuses[SIDE(bank)] & SIDE_STATUS_SPIKES && GetMonAbility(GetBankPartyData(bank)) != ABILITY_MAGICGUARD && ITEM_EFFECT(bank) != ITEM_EFFECT_HEAVY_DUTY_BOOTS)
	{
...
u16 GetTeamSpeedAverage(u8 bank)
{
	...
	{
		if (party[i].species == SPECIES_NONE || GetMonData(&party[i], MON_DATA_IS_EGG, NULL))
			continue;
...
u16 GetPokemonOnSideSpeedAverage(u8 bank)
{
	...
	if (IS_DOUBLE_BATTLE && BATTLER_ALIVE(PARTNER(bank)))
	{
...
u16 GetBattleMonMove(u8 bank, u8 i)
{
	...
	break_func(BATTLE_HISTORY->usedMoves) // Should throw error. We never want this to exist
		...
		move = BATTLE_HISTORY->usedMoves[bank][i];
	else move = gBattleMons[bank].moves[i];
#endif
...
{
	if (IsOfType(bank, TYPE_GHOST) || (switching && ITEM_EFFECT(bank) == ITEM_EFFECT_SHED_SHELL) || (!switching && ABILITY(bank) == ABILITY_RUNAWAY) || (!switching && ITEM_EFFECT(bank) == ITEM_EFFECT_CAN_ALWAYS_RUN))
		return FALSE;
	else
	{
		if (gBattleMons[bank].status2 & (STATUS2_ESCAPE_PREVENTION | STATUS2_WRAPPED) || (ABILITY_ON_OPPOSING_FIELD(bank, ABILITY_SHADOWTAG) && ABILITY(bank) != ABILITY_SHADOWTAG) || (ABILITY_ON_OPPOSING_FIELD(bank, ABILITY_ARENATRAP) && CheckGrounding(bank) == GROUNDED) || (ABILITY_ON_OPPOSING_FIELD(bank, ABILITY_MAGNETPULL) && IsOfType(bank, TYPE_STEEL)) || gStatuses3[bank] & (STATUS3_ROOTED | STATUS3_SKY_DROP_TARGET) || IsFairyLockActive())
			return TRUE;
	}
...
bool8 IsTakingSecondaryDamage(u8 bank)
{
	...
	{
		if (TakesDamageFromSandstorm(bank) || TakesDamageFromHail(bank) || gWishFutureKnock.futureSightCounter[bank] == 1 || gStatuses3[bank] & STATUS3_LEECHSEED || (gBattleMons[bank].status1 & STATUS1_PSN_ANY && ability != ABILITY_POISONHEAL) || gBattleMons[bank].status1 & STATUS1_BURN || ((gBattleMons[bank].status1 & STATUS1_SLEEP) > 1 && gBattleMons[bank].status2 & STATUS2_NIGHTMARE) || gBattleMons[bank].status2 & (STATUS2_CURSED | STATUS2_WRAPPED) || (BankSideHasSeaOfFire(bank) && !IsOfType(bank, TYPE_FIRE)) || (BankSideHasGMaxVineLash(bank) && !IsOfType(bank, TYPE_GRASS)) || (BankSideHasGMaxWildfire(bank) && !IsOfType(bank, TYPE_FIRE)) || (BankSideHasGMaxCannonade(bank) && !IsOfType(bank, TYPE_WATER)) || BankSideHasGMaxVolcalith(bank))
			return TRUE;
	}
...
bool8 WillFaintFromSecondaryDamage(u8 bank)
{
	...
	if (gStatuses3[bank] & STATUS3_PERISH_SONG && gDisableStructs[bank].perishSongTimer == 0)
		return TRUE;
	...
	if (ability != ABILITY_MAGICGUARD)
	{
		...
		if (gWishFutureKnock.weatherDuration != 1)
			damage += GetSandstormDamage(bank) + GetHailDamage(bank); // Weather's not about to end
		...
		if (damage + GetLeechSeedDamage(bank) + GetPoisonDamage(bank) + GetBurnDamage(bank) + GetCurseDamage(bank) + GetSeaOfFireDamage(bank) // Sea of Fire runs on last turn
				+ GetGMaxVineLashDamage(bank) + GetGMaxWildfireDamage(bank) + GetGMaxCannonadeDamage(bank) + GetGMaxVolcalithDamage(bank) >=
			hp)
			return TRUE;
	}
...
bool8 BadIdeaToPutToSleep(u8 bankDef, u8 bankAtk)
{
	...
	return !CanBePutToSleep(bankDef, TRUE) || gStatuses3[bankDef] & STATUS3_YAWN || defItemEffect == ITEM_EFFECT_CURE_SLP || defItemEffect == ITEM_EFFECT_CURE_STATUS || defAbility == ABILITY_EARLYBIRD || defAbility == ABILITY_SHEDSKIN || (defAbility == ABILITY_SYNCHRONIZE && CanBePutToSleep(bankAtk, TRUE)) || (defAbility == ABILITY_HYDRATION && gBattleWeather & WEATHER_RAIN_ANY && gWishFutureKnock.weatherDuration != 1) || (IS_DOUBLE_BATTLE && BATTLER_ALIVE(PARTNER(bankDef)) && ABILITY(PARTNER(bankDef)) == ABILITY_HEALER);
}
...
bool8 BadIdeaToPoison(u8 bankDef, u8 bankAtk)
{
	...
	return !CanBePoisoned(bankDef, bankAtk, TRUE) || defItemEffect == ITEM_EFFECT_CURE_PSN || defItemEffect == ITEM_EFFECT_CURE_STATUS || defAbility == ABILITY_SHEDSKIN || defAbility == ABILITY_POISONHEAL || defAbility == ABILITY_MAGICGUARD || defAbility == ABILITY_QUICKFEET || (defAbility == ABILITY_SYNCHRONIZE && CanBePoisoned(bankAtk, bankDef, TRUE) && !GoodIdeaToPoisonSelf(bankAtk)) || (defAbility == ABILITY_MARVELSCALE && PhysicalMoveInMoveset(bankAtk)) || (defAbility == ABILITY_NATURALCURE && CAN_SWITCH_OUT(bankDef)) || (defAbility == ABILITY_TOXICBOOST && PhysicalMoveInMoveset(bankDef)) || (defAbility == ABILITY_GUTS && PhysicalMoveInMoveset(bankDef)) || (atkAbility == ABILITY_POISONTOUCH && ContactMovesThatAffectTargetInMoveset(bankAtk, bankDef)) // Just poison it using attacker's ability
		   || (defAbility == ABILITY_HYDRATION && gBattleWeather & WEATHER_RAIN_ANY && gWishFutureKnock.weatherDuration != 1) || (IS_DOUBLE_BATTLE && BATTLER_ALIVE(PARTNER(bankDef)) && ABILITY(PARTNER(bankDef)) == ABILITY_HEALER) || MoveInMoveset(MOVE_FACADE, bankDef) || MoveInMoveset(MOVE_PSYCHOSHIFT, bankDef);
}
...
bool8 GoodIdeaToPoisonSelf(u8 bankAtk)
{
	...
	return CanBePoisoned(bankAtk, bankAtk, FALSE) && (atkAbility == ABILITY_MARVELSCALE || atkAbility == ABILITY_POISONHEAL || atkAbility == ABILITY_QUICKFEET || atkAbility == ABILITY_MAGICGUARD || (atkAbility == ABILITY_TOXICBOOST && PhysicalMoveInMoveset(bankAtk)) || (atkAbility == ABILITY_GUTS && PhysicalMoveInMoveset(bankAtk)) || MoveInMoveset(MOVE_FACADE, bankAtk) || MoveInMoveset(MOVE_PSYCHOSHIFT, bankAtk));
}
...
bool8 BadIdeaToParalyze(u8 bankDef, u8 bankAtk)
{
	...
	return !CanBeParalyzed(bankDef, TRUE) || defItemEffect == ITEM_EFFECT_CURE_PAR || defItemEffect == ITEM_EFFECT_CURE_STATUS || defAbility == ABILITY_SHEDSKIN || defAbility == ABILITY_QUICKFEET || (defAbility == ABILITY_SYNCHRONIZE && CanBeParalyzed(bankAtk, TRUE) && !GoodIdeaToParalyzeSelf(bankAtk)) || (defAbility == ABILITY_MARVELSCALE && PhysicalMoveInMoveset(bankAtk)) || (defAbility == ABILITY_NATURALCURE && CAN_SWITCH_OUT(bankDef)) || (defAbility == ABILITY_GUTS && PhysicalMoveInMoveset(bankDef)) || (defAbility == ABILITY_HYDRATION && gBattleWeather & WEATHER_RAIN_ANY && gWishFutureKnock.weatherDuration != 1) || (IS_DOUBLE_BATTLE && BATTLER_ALIVE(PARTNER(bankDef)) && ABILITY(PARTNER(bankDef)) == ABILITY_HEALER) || MoveInMoveset(MOVE_FACADE, bankDef) || MoveInMoveset(MOVE_PSYCHOSHIFT, bankDef);
}
...
bool8 GoodIdeaToParalyzeSelf(u8 bankAtk)
{
	...
	return CanBeParalyzed(bankAtk, FALSE) && (atkAbility == ABILITY_MARVELSCALE || atkAbility == ABILITY_QUICKFEET || (atkAbility == ABILITY_GUTS && PhysicalMoveInMoveset(bankAtk)) || MoveInMoveset(MOVE_FACADE, bankAtk) || MoveInMoveset(MOVE_PSYCHOSHIFT, bankAtk));
}
...
bool8 BadIdeaToBurn(u8 bankDef, u8 bankAtk)
{
	...
	return !CanBeBurned(bankDef, TRUE) || defItemEffect == ITEM_EFFECT_CURE_BRN || defItemEffect == ITEM_EFFECT_CURE_STATUS || defAbility == ABILITY_SHEDSKIN || defAbility == ABILITY_MAGICGUARD || defAbility == ABILITY_QUICKFEET || (defAbility == ABILITY_SYNCHRONIZE && CanBeBurned(bankAtk, TRUE) && !GoodIdeaToBurnSelf(bankAtk)) || (defAbility == ABILITY_MARVELSCALE && PhysicalMoveInMoveset(bankAtk)) || (defAbility == ABILITY_NATURALCURE && CAN_SWITCH_OUT(bankDef)) || (defAbility == ABILITY_FLAREBOOST && SpecialMoveInMoveset(bankDef)) || (defAbility == ABILITY_GUTS && PhysicalMoveInMoveset(bankDef)) || (defAbility == ABILITY_HYDRATION && gBattleWeather & WEATHER_RAIN_ANY && gWishFutureKnock.weatherDuration != 1) || (IS_DOUBLE_BATTLE && BATTLER_ALIVE(PARTNER(bankDef)) && ABILITY(PARTNER(bankDef)) == ABILITY_HEALER) || MoveInMoveset(MOVE_FACADE, bankDef) || MoveInMoveset(MOVE_PSYCHOSHIFT, bankDef);
}
...
bool8 GoodIdeaToBurnSelf(u8 bankAtk)
{
	...
	return CanBeBurned(bankAtk, FALSE) && (atkAbility == ABILITY_QUICKFEET || atkAbility == ABILITY_HEATPROOF || atkAbility == ABILITY_MAGICGUARD || (atkAbility == ABILITY_FLAREBOOST && SpecialMoveInMoveset(bankAtk)) || (atkAbility == ABILITY_GUTS && PhysicalMoveInMoveset(bankAtk)) || MoveInMoveset(MOVE_FACADE, bankAtk) || MoveInMoveset(MOVE_PSYCHOSHIFT, bankAtk));
}
...
bool8 BadIdeaToFreeze(u8 bankDef, u8 bankAtk)
{
	...
	return !CanBeFrozen(bankDef, TRUE) || defItemEffect != ITEM_EFFECT_CURE_FRZ || defItemEffect != ITEM_EFFECT_CURE_STATUS || (defAbility == ABILITY_SYNCHRONIZE && CanBeFrozen(bankAtk, TRUE)) || (defAbility == ABILITY_NATURALCURE && CAN_SWITCH_OUT(bankDef)) // Don't waste a one-time freeze
...
bool8 GoodIdeaToLowerAttack(u8 bankDef, u8 bankAtk, u16 move)
{
	...
	return STAT_STAGE(bankDef, STAT_STAGE_ATK) > 4 && PhysicalMoveInMoveset(bankDef) && defAbility != ABILITY_CONTRARY && defAbility != ABILITY_CLEARBODY && defAbility != ABILITY_WHITESMOKE
...
bool8 GoodIdeaToLowerDefense(u8 bankDef, u8 bankAtk, u16 move)
{
	...
	return STAT_STAGE(bankDef, STAT_STAGE_DEF) > 4 && PhysicalMoveInMoveset(bankAtk) && defAbility != ABILITY_CONTRARY && defAbility != ABILITY_CLEARBODY && defAbility != ABILITY_WHITESMOKE
		   //&& defAbility != ABILITY_FULLMETALBODY
...
bool8 GoodIdeaToLowerSpAtk(u8 bankDef, u8 bankAtk, u16 move)
{
	...
	return STAT_STAGE(bankDef, STAT_STAGE_SPATK) > 4 && SpecialMoveInMoveset(bankDef) && defAbility != ABILITY_CONTRARY && defAbility != ABILITY_CLEARBODY
		   //&& defAbility != ABILITY_FULLMETALBODY
...
bool8 GoodIdeaToLowerSpDef(u8 bankDef, u8 bankAtk, u16 move)
{
	...
	return STAT_STAGE(bankDef, STAT_STAGE_SPDEF) > 4 && SpecialMoveInMoveset(bankAtk) && defAbility != ABILITY_CONTRARY && defAbility != ABILITY_CLEARBODY
		   //&& defAbility != ABILITY_FULLMETALBODY
		   && defAbility != ABILITY_WHITESMOKE;
}
...
bool8 GoodIdeaToLowerSpeed(u8 bankDef, u8 bankAtk, u16 move)
{
	...
	return SpeedCalc(bankAtk) <= SpeedCalc(bankDef) && defAbility != ABILITY_CONTRARY && defAbility != ABILITY_CLEARBODY
		   //&& defAbility != ABILITY_FULLMETALBODY
		   && defAbility != ABILITY_WHITESMOKE;
}
...
bool8 GoodIdeaToLowerAccuracy(u8 bankDef, u8 bankAtk, u16 move)
{
	...
	return defAbility != ABILITY_CONTRARY && defAbility != ABILITY_CLEARBODY && defAbility != ABILITY_WHITESMOKE
		   //&& defAbility != ABILITY_FULLMETALBODY
...
bool8 GoodIdeaToLowerEvasion(u8 bankDef, u8 bankAtk, unusedArg u16 move)
{
	...
	return (STAT_STAGE(bankDef, STAT_STAGE_EVASION) > 6 || MoveInMovesetWithAccuracyLessThan(bankAtk, bankDef, 90, TRUE)) && defAbility != ABILITY_CONTRARY && defAbility != ABILITY_CLEARBODY
		   //&& defAbility != ABILITY_FULLMETALBODY
		   && defAbility != ABILITY_WHITESMOKE;
}
...
bool8 IsMovePredictionHealingMove(u8 bankAtk, u8 bankDef)
{
	...
	{
		u8 effect = gBattleMoves[move].effect;
		return effect == EFFECT_RESTORE_HP || effect == EFFECT_MORNING_SUN || effect == EFFECT_SWALLOW || effect == EFFECT_WISH;
	}
...
bool8 DamagingMoveInMoveset(u8 bank)
{
	...
	for (int i = 0; i < MAX_MON_MOVES; ++i)
	{
		...
		if (!(gBitTable[i] & moveLimitations))
		{
			if (SPLIT(move) != SPLIT_STATUS && gBattleMoves[move].power != 0 && gBattleMoves[move].effect != EFFECT_COUNTER && gBattleMoves[move].effect != EFFECT_MIRROR_COAT)
				return TRUE;
		}
	}
...
bool8 PhysicalMoveInMoveset(u8 bank)
{
	...
	for (int i = 0; i < MAX_MON_MOVES; ++i)
	{
		...
		if (!(gBitTable[i] & moveLimitations))
		{
			if (CalcMoveSplit(bank, move) == SPLIT_PHYSICAL && gBattleMoves[move].power != 0 && gBattleMoves[move].effect != EFFECT_COUNTER)
				return TRUE;
		}
	}
...
bool8 SpecialMoveInMoveset(u8 bank)
{
	...
	for (int i = 0; i < MAX_MON_MOVES; ++i)
	{
		...
		if (!(gBitTable[i] & moveLimitations))
		{
			if (CalcMoveSplit(bank, move) == SPLIT_SPECIAL && gBattleMoves[move].power != 0 && gBattleMoves[move].effect != EFFECT_MIRROR_COAT)
				return TRUE;
		}
	}
...
bool8 PhysicalMoveInMonMoveset(struct Pokemon *mon, u8 moveLimitations)
{
	...
	for (int i = 0; i < MAX_MON_MOVES; ++i)
	{
		...
		if (!(gBitTable[i] & moveLimitations))
		{
			if (CalcMoveSplitFromParty(mon, move) == SPLIT_PHYSICAL && gBattleMoves[move].power != 0 && gBattleMoves[move].effect != EFFECT_COUNTER)
				return TRUE;
		}
	}
...
bool8 SpecialMoveInMonMoveset(struct Pokemon *mon, u8 moveLimitations)
{
	...
	for (int i = 0; i < MAX_MON_MOVES; ++i)
	{
		...
		if (!(gBitTable[i] & moveLimitations))
		{
			if (CalcMoveSplitFromParty(mon, move) == SPLIT_SPECIAL && gBattleMoves[move].power != 0 && gBattleMoves[move].effect != EFFECT_MIRROR_COAT)
				return TRUE;
		}
	}
	...
	for (int i = 0; i < MAX_MON_MOVES; ++i)
	{
		...
		if (!(gBitTable[i] & moveLimitations))
		{
			if (gBattleMoves[gBattleMons[bank].moves[i]].effect == EFFECT_PROTECT)
			{
				...
				case MOVE_MATBLOCK:
					if (checkType & CHECK_MAT_BLOCK && gDisableStructs[bank].isFirstTurn)
						return TRUE;
					break;
				}
			}
...
bool8 DamagingMoveTypeInMoveset(u8 bank, u8 moveType)
{
	...
	for (int i = 0; i < MAX_MON_MOVES; ++i)
	{
		...
		if (!(gBitTable[i] & moveLimitations))
		{
			if (GetMoveTypeSpecial(bank, move) == moveType && SPLIT(move) != SPLIT_STATUS)
				return TRUE;
		}
	}
...
bool8 DamagingAllHitMoveTypeInMoveset(u8 bank, u8 moveType)
{
	...
	for (int i = 0; i < MAX_MON_MOVES; ++i)
	{
		...
		if (!(gBitTable[i] & moveLimitations))
		{
			if (GetMoveTypeSpecial(bank, move) == moveType && SPLIT(move) != SPLIT_STATUS && gBattleMoves[move].target & MOVE_TARGET_ALL)
				return TRUE;
		}
	}
...
bool8 DamagingSpreadMoveInMoveset(u8 bank)
{
	...
	for (int i = 0; i < MAX_MON_MOVES; ++i)
	{
		...
		if (!(gBitTable[i] & moveLimitations))
		{
			if (SPLIT(move) != SPLIT_STATUS && gBattleMoves[move].target & (MOVE_TARGET_BOTH | MOVE_TARGET_ALL))
				return TRUE;
		}
	}
...
bool8 HealingMoveInMoveset(u8 bank)
{
	...
	for (int i = 0; i < MAX_MON_MOVES; ++i)
	{
		...
		{
			u8 effect = gBattleMoves[move].effect;
			if (effect == EFFECT_RESTORE_HP || effect == EFFECT_MORNING_SUN || effect == EFFECT_SWALLOW || effect == EFFECT_WISH)
				return TRUE;
		}
	}
...
bool8 ContactMovesThatAffectTargetInMoveset(u8 bankAtk, u8 bankDef)
{
	...
	for (int i = 0; i < MAX_MON_MOVES; ++i)
	{
		...
		if (!(gBitTable[i] & moveLimitations))
		{
			if (CheckContact(move, bankAtk) && !(AI_SpecialTypeCalc(move, bankDef, bankAtk) & MOVE_RESULT_NO_EFFECT))
				return TRUE;
		}
	}
...
{
	if (CanDynamax(bankAtk) && (ITEM_EFFECT(bankAtk) == ITEM_EFFECT_CHOICE_BAND || ABILITY(bankAtk) == ABILITY_GORILLATACTICS) && CHOICED_MOVE(bankAtk) != MOVE_NONE && CHOICED_MOVE(bankAtk) != 0xFFFF) // AI is locked into some move
	{
...
static bool8 CalcOnlyBadMovesLeftInMoveset(u8 bankAtk, u8 bankDef)
{
	...
	for (i = 0; i < MAX_MON_MOVES; ++i)
	{
		...
		if (!(gBitTable[i] & moveLimitations))
		{
			...
			if (viability >= 100)
			{
				...
				if (gBattleMoves[move].effect == EFFECT_KNOCK_OFF && CanKnockOffItem(bankDef))
	...
	if (IsDoubleBattle() || !MoveEffectInMoveset(EFFECT_PROTECT, bankAtk)) // Single battle protector usually protect to stall
	{
		if (numDamageMoves == 0)
		{
			if (IS_BEHIND_SUBSTITUTE(bankAtk) && !DamagingMoveThaCanBreakThroughSubstituteInMoveset(bankDef, bankAtk) && IsTakingSecondaryDamage(bankDef))
		...
		else if (IS_BEHIND_SUBSTITUTE(bankAtk) && !DamagingMoveThaCanBreakThroughSubstituteInMoveset(bankDef, bankAtk))
		{
				 ...
				 /*&& numDamageMoves == 1*/)																									 // Only 1 viable damaging move left
		{
			...
			if (dmg < gBattleMons[bankDef].hp) // Move doesn't KO
			{
				...
				if (GetMostSuitableMonToSwitchIntoScore() & SWITCHING_INCREASE_HAS_SUPER_EFFECTIVE_MOVE)
				{
					...
					if (GetMostSuitableMonToSwitchIntoScore() == SWITCHING_INCREASE_HAS_SUPER_EFFECTIVE_MOVE && WallsFoe(bankAtk, bankDef))
...
u16 TryReplaceMoveWithZMove(u8 bankAtk, u8 bankDef, u16 move)
{
	...
	if (!gNewBS->zMoveData.used[bankAtk] && SPLIT(move) != SPLIT_STATUS && ShouldAIUseZMove(bankAtk, bankDef, move))
	{
...
u8 GetAIMoveEffectForMaxMove(u16 move, u8 bankAtk, u8 bankDef)
{
	...
	case MAX_EFFECT_RAISE_TEAM_SP_DEF:
		if (STAT_STAGE(bankAtk, (maxEffect - MAX_EFFECT_RAISE_TEAM_ATTACK) + 1) < STAT_STAGE_MAX || (IS_DOUBLE_BATTLE && BATTLER_ALIVE(PARTNER(bankAtk)) && STAT_STAGE(PARTNER(bankAtk), (maxEffect - MAX_EFFECT_RAISE_TEAM_ATTACK) + 1) < STAT_STAGE_MAX))
	...
	case MAX_EFFECT_PSYCHIC_TERRAIN:
		if (!(gBattleTypeFlags & BATTLE_TYPE_BATTLE_CIRCUS && gBattleCircusFlags & BATTLE_CIRCUS_TERRAIN) && gTerrainType != (maxEffect - MAX_EFFECT_ELECTRIC_TERRAIN) + 1)
	...
	case MAX_EFFECT_RECYCLE_BERRIES:
		if (SAVED_CONSUMED_ITEMS(bankAtk) != ITEM_NONE && IsBerry(SAVED_CONSUMED_ITEMS(bankAtk)) && ITEM(bankAtk) == ITEM_NONE)
	...
	case MAX_EFFECT_DEFOG:
		if (gSideStatuses[SIDE(bankDef)] & (SIDE_STATUS_REFLECT | SIDE_STATUS_LIGHTSCREEN | SIDE_STATUS_SAFEGUARD | SIDE_STATUS_MIST) || gNewBS->AuroraVeilTimers[SIDE(bankDef)] != 0 || gSideStatuses[SIDE(bankAtk)] & SIDE_STATUS_SPIKES || !(gSideStatuses[SIDE(bankDef)] & SIDE_STATUS_SPIKES))
...
bool8 TeamFullyHealedMinusBank(u8 bank)
{
	...
	{
		if (party[i].species == SPECIES_NONE || GetMonData(&party[i], REQ_EGG, NULL) || i == gBattlerPartyIndexes[bank])
			continue;
...
bool8 AnyUsefulStatIsRaised(u8 bank)
{
	...
	if (gBattleResources->flags->flags[bank] & 1 && DamagingMoveTypeInMoveset(bank, TYPE_FIRE))
		return TRUE;
...
bool8 ShouldAIUseZMove(u8 bankAtk, u8 bankDef, u16 move)
{
	...
	if (zMove != MOVE_NONE)
	{
		if (zMove != 0xFFFF) // Damaging Z-Move
		{
			...
			if (MoveBlockedBySubstitute(zMove, bankAtk, bankDef) || (defMovePrediction == MOVE_SUBSTITUTE && !MoveWouldHitFirst(zMove, bankAtk, bankDef) && !MoveIgnoresSubstitutes(zMove, ABILITY(bankAtk))))
			...
			if (defMovePrediction == MOVE_PROTECT || defMovePrediction == MOVE_KINGSSHIELD || defMovePrediction == MOVE_SPIKYSHIELD || defMovePrediction == MOVE_OBSTRUCT || (IsDynamaxed(bankDef) && SPLIT(defMovePrediction) == SPLIT_STATUS))
			...
			if (IsRaidBattle() && gNewBS->dynamaxData.raidShieldsUp && SIDE(bankAtk) == B_SIDE_PLAYER && SIDE(bankDef) == B_SIDE_OPPONENT) // Partner AI on Raid Pokemon with shields up
			{
				...
				if (SPLIT(partnerMove) == SPLIT_STATUS || MoveWouldHitFirst(partnerMove, bankAtkPartner, bankAtk) || (gChosenMovesByBanks[bankAtkPartner] != MOVE_NONE && gBattleStruct->moveTarget[bankAtkPartner] != bankDef)) // Not targeting raid Pokemon
		...
		else // Status Move
		{
			...
			case Z_EFFECT_ATK_UP_3:
				if (STAT_CAN_RISE(bankAtk, STAT_STAGE_ATK) && MoveSplitInMoveset(bankAtk, SPLIT_PHYSICAL))
					return TRUE;
				break;
			...
			case Z_EFFECT_SPATK_UP_3:
				if (STAT_CAN_RISE(bankAtk, STAT_STAGE_SPATK) && MoveSplitInMoveset(bankAtk, SPLIT_SPECIAL))
					return TRUE;
				break;
			...
			case Z_EFFECT_ACC_UP_3:
				if (STAT_CAN_RISE(bankAtk, STAT_STAGE_ACC) && MoveInMovesetWithAccuracyLessThan(bankAtk, bankDef, 90, FALSE))
					return TRUE;
				break;
...
void CalcAIDynamaxMon(u8 bank)
{
	...
	if (gBattleTypeFlags & BATTLE_TYPE_DYNAMAX && !gNewBS->dynamaxData.used[bank])
	{
		...
		for (i = 0, bestMonScore = 0, bestStatAmount = 0; i < PARTY_SIZE; ++i) // Do entire party at once, even for Multi Battles
		{
			...
			if (mon->species == SPECIES_NONE || GetMonData(mon, MON_DATA_HP, NULL) == 0 || GetMonData(mon, MON_DATA_IS_EGG, NULL) || !MonCanDynamax(mon))
				continue;
			...
			{
				if (updateScore == bestMonScore && bestMonStat <= bestStatAmount)
	...
	if (bankAtk != bankDef && CanDynamax(bankAtk))
	{
		...
		if (gNewBS->ai.dynamaxMonId[SIDE(bankAtk)] == gBattlerPartyIndexes[bankAtk])
		{
			if (AI_THINKING_STRUCT->aiFlags > AI_SCRIPT_CHECK_BAD_MOVE || SIDE(bankAtk) == B_SIDE_PLAYER) // Smart AI or partners only
			{
				...
				if (predictedMove != MOVE_NONE && MoveWouldHitFirst(predictedMove, bankAtk, bankDef) && MoveKnocksOutXHits(predictedMove, bankAtk, bankDef, 1))
					return; // Just KO the opponent normally

				if ((IS_SINGLE_BATTLE && ViableMonCountFromBank(bankAtk) > 1) || (IS_DOUBLE_BATTLE && ViableMonCountFromBank(bankAtk) > 2)) // Could use Dynamax on another Pokemon
				{
					predictedMove = IsValidMovePrediction(bankDef, bankAtk);
					if (predictedMove != MOVE_NONE && MoveWouldHitFirst(predictedMove, bankDef, bankAtk) && MoveKnocksOutAfterDynamax(predictedMove, bankDef, bankAtk))
						return; // Don't Dynamax is foe will just KO you
				}

				OnlyBadMovesLeftInMoveset(bankAtk, bankDef); // Force calculation

				if ((ITEM_EFFECT(bankAtk) == ITEM_EFFECT_CHOICE_BAND || ABILITY(bankAtk) == ABILITY_GORILLATACTICS) && !gNewBS->ai.shouldFreeChoiceLockWithDynamax[bankAtk][bankDef]) // There are good moves left
