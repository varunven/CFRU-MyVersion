...
void BattleAI_HandleItemUseBeforeAISetup(void)
{
	...
	if ((gBattleTypeFlags & BATTLE_TYPE_TRAINER) && !(gBattleTypeFlags & (BATTLE_TYPE_LINK | BATTLE_TYPE_SAFARI | BATTLE_TYPE_FRONTIER | BATTLE_TYPE_TWO_OPPONENTS | BATTLE_TYPE_INGAME_PARTNER | BATTLE_TYPE_EREADER_TRAINER)) && gTrainerBattleOpponent_A != SECRET_BASE_OPPONENT && !IsFrontierTrainerId(gTrainerBattleOpponent_A))
	{
		for (i = 0; i < 4; i++)
		{
...
static u8 ChooseMoveOrAction_Doubles(struct AIScript *aiScriptData)
{
	...
	u32 mostDamage = (actionOrMoveIndex[0] < MAX_MON_MOVES && SPLIT(actionOrMoveIndex[0]) != SPLIT_STATUS) ? GetFinalAIMoveDamage(actionOrMoveIndex[0], gBankAttacker, mostViableTargetsArray[0], 1, NULL) : 0;
	...
	for (i = 1; i < MAX_BATTLERS_COUNT; i++)
	{
		if (bestMovePointsForTarget[i] == mostMovePoints)
		{
			...
			if (move < MAX_MON_MOVES // Move was chosen
				&& pastMove < MAX_MON_MOVES && SPLIT(move) != SPLIT_STATUS && SPLIT(pastMove) != SPLIT_STATUS)
			{
		...
		if (ShouldSwitch()) // 0x8039A80
		{
			if (gBattleStruct->switchoutIndex[SIDE(gActiveBattler)] == PARTY_SIZE)
			{
				...
				if (monToSwitchId == PARTY_SIZE)
				{
					...
					{
						if (party[monToSwitchId].hp == 0 || GetMonData(&party[monToSwitchId], MON_DATA_IS_EGG, 0) || monToSwitchId == gBattlerPartyIndexes[battlerIn1] || monToSwitchId == gBattlerPartyIndexes[battlerIn2] || monToSwitchId == gBattleStruct->monToSwitchIntoId[battlerIn1] || monToSwitchId == gBattleStruct->monToSwitchIntoId[battlerIn2])
							continue;
...
static bool8 ShouldSwitch(void)
{
	...
	{
		if (party[i].hp == 0 || GetMonData(&party[i], MON_DATA_SPECIES2, 0) == SPECIES_NONE || GetMonData(&party[i], MON_DATA_IS_EGG, 0) || i == gBattlerPartyIndexes[battlerIn1] || i == gBattlerPartyIndexes[battlerIn2] || i == gBattleStruct->monToSwitchIntoId[battlerIn1] || i == gBattleStruct->monToSwitchIntoId[battlerIn2])
			continue;
...
static bool8 PredictedMoveWontDoTooMuchToMon(u8 activeBattler, struct Pokemon *mon, u8 foe)
{
	...
	return predictedDmg < mon->maxHP / 2 && HealingMoveInMonMoveset(mon) && SpeedCalcMon(SIDE(activeBattler), mon) > SpeedCalc(foe); // Has time to heal
}
...
static bool8 ShouldSwitchIfOnlyBadMovesLeft(void)
{
	...
	if (IS_DOUBLE_BATTLE)
	{
		if ((!BATTLER_ALIVE(foe1) || OnlyBadMovesLeftInMoveset(gActiveBattler, foe1)) && (!BATTLER_ALIVE(foe2) || OnlyBadMovesLeftInMoveset(gActiveBattler, foe2)))
		{
...
static bool8 FindMonThatAbsorbsOpponentsMove(void)
{
	...
	if (IS_SINGLE_BATTLE)
	{
		...
		else
		{
			if (!CanKnockOut(foe1, gActiveBattler)												   // The enemy can't KO you first
				&& CanKnockOut(gActiveBattler, foe1) && AnyStatGreaterThan(gActiveBattler, 6 + 0)) // Has stat boosts +7 or more
	...
	if (((predictedMove1 == MOVE_NONE || predictedMove1 == MOVE_PREDICTION_SWITCH) && (predictedMove2 == MOVE_NONE || predictedMove2 == MOVE_PREDICTION_SWITCH)) || (SPLIT(predictedMove1) == SPLIT_STATUS && SPLIT(predictedMove2) == SPLIT_STATUS))
		return FALSE;
	...
	case TYPE_GRASS:
		...
		break;
	case TYPE_FAIRY:
		absorbingTypeAbility1 = ABILITY_GUARDIANANGEL;
		absorbingTypeAbility2 = ABILITY_GUARDIANANGEL;
		absorbingTypeAbility3 = ABILITY_GUARDIANANGEL;
		break;
	case TYPE_ROCK:
		absorbingTypeAbility1 = ABILITY_MOUNTAINEER;
	...
	if (atkAbility == absorbingTypeAbility1 || atkAbility == absorbingTypeAbility2 || atkAbility == absorbingTypeAbility3)
		return FALSE;
	...
	for (i = firstId; i < lastId; i++)
	{
		...
		if (party[i].hp == 0 || species == SPECIES_NONE || GetMonData(&party[i], MON_DATA_IS_EGG, 0) || i == gBattlerPartyIndexes[battlerIn1] || i == gBattlerPartyIndexes[battlerIn2] || i == gBattleStruct->monToSwitchIntoId[battlerIn1] || i == gBattleStruct->monToSwitchIntoId[battlerIn2])
			continue;

		if (monAbility == absorbingTypeAbility1 || monAbility == absorbingTypeAbility2 || monAbility == absorbingTypeAbility3)
		{
			// we found a mon.
...
static bool8 ShouldSwitchIfNaturalCureOrRegenerator(void)
{
	...
	case ABILITY_NATURALCURE:
		...
		if (gBattleMons[gActiveBattler].status1 // Has regular status and over half health
			&& gBattleMons[gActiveBattler].hp >= gBattleMons[gActiveBattler].maxHP / 2 && !IsDynamaxed(gActiveBattler))
			break;
		return FALSE;

	// Try switch if less than half health, enemy can kill, and mon can't kill enemy first
	case ABILITY_REGENERATOR:
		if (gBattleMons[gActiveBattler].hp > gBattleMons[gActiveBattler].maxHP / 2 || IsDynamaxed(gActiveBattler))
			return FALSE;
...
static bool8 PassOnWish(void)
{
	...
	if (gWishFutureKnock.wishCounter[bank])
	{
		...
		if (gBattleMons[bank].hp < gBattleMons[bank].maxHP / 2 && ((!CanKnockOut(opposingBattler1, bank) && !(IS_DOUBLE_BATTLE && CanKnockOut(opposingBattler2, bank))) || HasProtectionMoveInMoveset(bank, CHECK_MAT_BLOCK)))
			return FALSE;
		...
		for (i = firstId; i < lastId; ++i)
		{
			if (party[i].hp == 0 || party[i].species == SPECIES_NONE || GetMonData(&party[i], MON_DATA_IS_EGG, 0) || i == gBattlerPartyIndexes[battlerIn1] || i == gBattlerPartyIndexes[battlerIn2] || i == gBattleStruct->monToSwitchIntoId[battlerIn1] || i == gBattleStruct->monToSwitchIntoId[battlerIn2])
				continue;
...
static u8 TheCalcForSemiInvulnerableTroll(u8 bankAtk, u8 flags, bool8 checkLockedMoves)
{
	...
	if (((gStatuses3[bankAtk] & STATUS3_SEMI_INVULNERABLE) || (checkLockedMoves && gLockedMoves[bankAtk] != MOVE_NONE)) && gBattleStruct->moveTarget[bankAtk] == gActiveBattler)
	{
...
static bool8 IsYawned(void)
{
	if (ABILITY(gActiveBattler) != ABILITY_NATURALCURE && gStatuses3[gActiveBattler] & STATUS3_YAWN && CanBePutToSleep(gActiveBattler, FALSE) // Could have been yawned and then afflicted with another status condition
		&& gBattleMons[gActiveBattler].hp > gBattleMons[gActiveBattler].maxHP / 4)
	{
		...
		for (int i = 0; i < gBattlersCount; ++i)
		{
			...
			{
				if (gBattleMons[i].status2 & STATUS2_WRAPPED && ABILITY(i) != ABILITY_MAGICGUARD // Taking trap damage
		...
		if (STAT_STAGE(gActiveBattler, STAT_STAGE_EVASION) >= 6 + 3 && !ABILITY_ON_OPPOSING_FIELD(gActiveBattler, ABILITY_UNAWARE) && !ABILITY_ON_OPPOSING_FIELD(gActiveBattler, ABILITY_KEENEYE))
			return FALSE;

		// Don't switch if you can fight through the sleep
		u8 ability = ABILITY(gActiveBattler);
		u8 itemEffect = ITEM_EFFECT(gActiveBattler);
		if (itemEffect == ITEM_EFFECT_CURE_SLP || itemEffect == ITEM_EFFECT_CURE_STATUS || ability == ABILITY_EARLYBIRD || ability == ABILITY_SHEDSKIN || MoveEffectInMoveset(EFFECT_SNORE, gActiveBattler) || MoveEffectInMoveset(EFFECT_SLEEP_TALK, gActiveBattler) || (CheckGrounding(gActiveBattler) && (MoveInMoveset(MOVE_ELECTRICTERRAIN, gActiveBattler) || MoveInMoveset(MOVE_MISTYTERRAIN, gActiveBattler))))
			return FALSE;
		...
		else // Double Battle
		{
			u16 bestMove1 = gNewBS->ai.bestDoublesKillingMoves[gActiveBattler][foe1];
			u16 bestMove2 = gNewBS->ai.bestDoublesKillingMoves[gActiveBattler][foe2];
			if (GetDoubleKillingScore(bestMove1, gActiveBattler, foe1) >= BEST_DOUBLES_KO_SCORE || GetDoubleKillingScore(bestMove2, gActiveBattler, foe2) >= BEST_DOUBLES_KO_SCORE)
				return FALSE; // Don't switch if this mon can do some major damage to the enemy side
		}
...
static bool8 IsTakingAnnoyingSecondaryDamage(void)
{
	if (GetPredictedAIAbility(gActiveBattler, FOE(gActiveBattler)) != ABILITY_MAGICGUARD && !CanKillAFoe(gActiveBattler) && !IsDynamaxed(gActiveBattler) && AI_THINKING_STRUCT->aiFlags > AI_SCRIPT_CHECK_BAD_MOVE) // Has smart AI
	{
		if ((gStatuses3[gActiveBattler] & STATUS3_LEECHSEED && (Random() & 3) == 0)																																														  // 25% chance to switch out when seeded
			|| ((gBattleMons[gActiveBattler].status1 & STATUS1_SLEEP) > 1 && gBattleMons[gActiveBattler].status2 & STATUS2_NIGHTMARE) || gBattleMons[gActiveBattler].status2 & (STATUS2_CURSED) || (gBattleMons[gActiveBattler].status1 & STATUS1_TOXIC_COUNTER) > 0x600) // Been sitting with toxic for 6 turns
		{
...
static bool8 ShouldSwitchToAvoidDeath(void)
{
	...
	if (IS_SINGLE_BATTLE && AI_THINKING_STRUCT->aiFlags > AI_SCRIPT_CHECK_BAD_MOVE) // Has smart AI
	{
		...
		if (defMove != MOVE_NONE																																										// Foe going to attack
			...
			&& (!IS_BEHIND_SUBSTITUTE(gActiveBattler) || !MoveBlockedBySubstitute(defMove, FOE(gActiveBattler), gActiveBattler)) && MoveKnocksOutXHits(defMove, FOE(gActiveBattler), gActiveBattler, 1) // Foe will kill
...
bool8 FindMonWithFlagsAndSuperEffective(u8 flags, u8 moduloPercent)
{
	...
	for (i = start; i < end; ++i)
	{
		if (party[i].hp == 0 || party[i].species == SPECIES_NONE || GetMonData(&party[i], MON_DATA_IS_EGG, NULL) || i == gBattlerPartyIndexes[battlerIn1] || i == gBattlerPartyIndexes[battlerIn2] || i == gBattleStruct->monToSwitchIntoId[battlerIn1] || i == gBattleStruct->monToSwitchIntoId[battlerIn2])
			continue;
...
static bool8 ShouldSwitchIfWonderGuard(void)
{
	...
	for (i = 0; i < MAX_MON_MOVES; ++i)
	{
		...
		if (!(gBitTable[i] & moveLimitations))
		{
			...
			if (SPLIT(move) != SPLIT_STATUS)
			{
				...
				if (atkAbility == ABILITY_MOLDBREAKER || atkAbility == ABILITY_TERAVOLT || atkAbility == ABILITY_TURBOBLAZE)
					return FALSE;
			...
			else if (!MoveBlockedBySubstitute(move, bankAtk, bankDef))
			{
				...
				case EFFECT_TRICK:
					if (move == MOVE_TRICK)
					{
						...
						if (CanTransferItem(atkSpecies, ITEM(bankAtk)) && CanTransferItem(atkSpecies, ITEM(bankDef)) && CanTransferItem(defSpecies, ITEM(bankAtk)) && CanTransferItem(defSpecies, ITEM(bankDef)))
						{
	...
	for (i = start; i < end; ++i)
	{
		if (party[i].hp == 0 || party[i].species == SPECIES_NONE || GetMonData(&party[i], MON_DATA_IS_EGG, 0) || i == gBattlerPartyIndexes[battlerIn1] || i == gBattleStruct->monToSwitchIntoId[battlerIn1])
			continue;
...
u8 GetMostSuitableMonToSwitchInto(void)
{
	...
	if (option1 == PARTY_SIZE || GetMonData(&party[option1], MON_DATA_HP, NULL) == 0) // Best mon is dead
	{
	...
	if (option1 == gBattleStruct->monToSwitchIntoId[battlerIn1] || option1 == gBattleStruct->monToSwitchIntoId[battlerIn2])
		return option2;
...
u8 CalcMostSuitableMonToSwitchInto(void)
{
	...
	for (i = firstId; i < lastId; ++i)
	{
		if (party[i].species != SPECIES_NONE && !GetMonData(&party[i], MON_DATA_IS_EGG, NULL) && party[i].hp > 0 && gBattlerPartyIndexes[battlerIn1] != i && gBattlerPartyIndexes[battlerIn2] != i && i != gBattleStruct->monToSwitchIntoId[battlerIn1] && i != gBattleStruct->monToSwitchIntoId[battlerIn2])
		{
			++count;
		}
	}
	...
	for (i = firstId; i < lastId; ++i)
	{
		if (party[i].species != SPECIES_NONE && party[i].hp > 0 && !GetMonData(&party[i], MON_DATA_IS_EGG, NULL) && i != gBattlerPartyIndexes[battlerIn1] && i != gBattlerPartyIndexes[battlerIn2] && i != gBattleStruct->monToSwitchIntoId[battlerIn1] && i != gBattleStruct->monToSwitchIntoId[battlerIn2])
		{
			u8 foes[] = {foe1, foe2};
			...
			canNegateToxicSpikes[i] = CheckGroundingFromPartyData(&party[i]) && (gBaseStats[species].type1 == TYPE_POISON || gBaseStats[species].type2 == TYPE_POISON);
			...
			for (j = 0; j < gBattlersCount / 2; ++j) // Loop through all enemies on field
			{
				...
				if (BATTLER_ALIVE(foe) && (j == 0 || foes[0] != foes[j])) // Don't check same opponent twice
				{
					...
					if (CanKnockOutFromParty(&party[i], foe, &damageData))
					{
						...
						if (ability == ABILITY_MOXIE || ability == ABILITY_SOULHEART || ability == ABILITY_BEASTBOOST)
						...
						else
						{
							for (k = 0; k < MAX_MON_MOVES; ++k)
							{
								...
								if (move == MOVE_FELLSTINGER && !(gBitTable[k] & moveLimitations))
								{
								...
								else if (SPLIT(move) != SPLIT_STATUS && PriorityCalcMon(&party[i], move) > 0 && MoveKnocksOutXHitsFromParty(move, &party[i], foe, 1, &damageData))
								{
					...
					else // This mon can't KO the foe
					{
						for (k = 0; k < MAX_MON_MOVES; ++k)
						{
							...
							if (move != MOVE_NONE && SPLIT(move) != SPLIT_STATUS && !(gBitTable[k] & moveLimitations) && TypeCalc(move, gActiveBattler, foe, &party[i], TRUE) & MOVE_RESULT_SUPER_EFFECTIVE)
							{
					...
					for (k = 0; k < MAX_MON_MOVES; ++k)
					{
						...
						if (gBattleMons[foe].status2 & STATUS2_MULTIPLETURNS && MoveInMoveset(gLockedMoves[foe], foe) && move != gLockedMoves[foe])
	...
	if (bestMonId != PARTY_SIZE)
	{
		if (scores[bestMonId] < 8)
		{
			...
			for (i = firstId; i < lastId; ++i)
			{
				if (canRemoveHazards[i] || (tSpikesActive && canNegateToxicSpikes[i]))
				{
					if (IS_DOUBLE_BATTLE)
					{
...
u32 WildMonIsSmart(unusedArg u8 bank)
{
	...
	if (IsMegaSpecies(species) || IsRedPrimalSpecies(species) || IsBluePrimalSpecies(species) || IsUltraNecrozmaSpecies(species))
...
void PopulateAIScriptStructWithBaseDefenderData(struct AIScript *data, u8 bankDef)
{
	...
	data->partnerHandling = IS_DOUBLE_BATTLE && BATTLER_ALIVE(data->bankAtkPartner) && !IsBankIncapacitated(data->bankAtkPartner) && gChosenMovesByBanks[data->bankAtkPartner] != MOVE_NONE // Partner actually selected a move
							&& gBattleStruct->moveTarget[data->bankAtkPartner] == bankDef && gBattleMoves[data->partnerMove].target & MOVE_TARGET_SELECTED									// Partner isn't using spread move
	...
	for (u8 bankAtk = 0; bankAtk < gBattlersCount; ++bankAtk)
	{
		if (BATTLER_ALIVE(bankAtk))
		{
			...
			for (u8 bankDef = 0; bankDef < gBattlersCount; ++bankDef)
			{
				...
				if (gBattleMons[bankAtk].status2 & STATUS2_RECHARGE || gDisableStructs[bankAtk].truantCounter != 0)
				{
					StoreMovePrediction(bankAtk, bankDef, MOVE_NONE);
				}
				else if (IsBankAsleep(bankAtk) && !MoveEffectInMoveset(EFFECT_SLEEP_TALK, bankAtk) && !MoveEffectInMoveset(EFFECT_SNORE, bankAtk)) // Can't get around sleep
				{
					StoreMovePrediction(bankAtk, bankDef, MOVE_NONE);
				}
				else if (gBattleMons[bankAtk].status2 & STATUS2_MULTIPLETURNS && MoveInMoveset(gLockedMoves[bankAtk], bankAtk)) // Still knows locked move
				{
...
static void UpdateStrongestMoves(void)
{
	...
	for (bankAtk = 0; bankAtk < gBattlersCount; ++bankAtk)
	{
		...
		if (!IS_TRANSFORMED(bankAtk) && !BankMegaEvolved(bankAtk, FALSE) && MegaEvolutionEnabled(bankAtk) && !DoesZMoveUsageStopMegaEvolution(bankAtk))
		{
		...
		case AI_ITEM_FULL_RESTORE:
			...
			FULL_RESTORE_LOGIC:
				...
				else if (gBattleMons[gActiveBattler].hp < gBattleMons[gActiveBattler].maxHP / 2) // Smart AI should only use at less than half health
				{
					...
					if ((BATTLER_ALIVE(foe) && ShouldRecover(gActiveBattler, foe, 0xFFFF)) || (IS_DOUBLE_BATTLE && BATTLER_ALIVE(PARTNER(foe)) && ShouldRecover(gActiveBattler, PARTNER(foe), 0xFFFF)))
					{
						shouldUse = TRUE;
					}
				}
...
{
#ifdef VAR_GAME_DIFFICULTY
	if (gBattleTypeFlags & BATTLE_TYPE_TRAINER && !IS_DOUBLE_BATTLE && gBattleScripting.battleStyle == OPTIONS_BATTLE_STYLE_SHIFT && BATTLER_ALIVE(GetBattlerAtPosition(B_POSITION_OPPONENT_LEFT)) // AI has a mon that can be switched out
