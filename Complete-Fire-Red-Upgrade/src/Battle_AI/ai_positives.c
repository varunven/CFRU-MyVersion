...
u8 AIScript_Positives(const u8 bankAtk, const u8 bankDef, const u16 originalMove, const u8 originalViability, struct AIScript *data)
{
	...
	if (!NO_MOLD_BREAKERS(atkAbility, move) && gMoldBreakerIgnoredAbilities[defAbility])
	...
	case EFFECT_EXPLOSION:
		...
		{
			if (data->atkItemEffect == ITEM_EFFECT_GEM && data->atkItemQuality == moveType) // AI Was meant to explode
			{
				INCREASE_VIABILITY(4); // Counteract the negative check
			}
			else if (data->atkItemEffect == ITEM_EFFECT_CUSTAP_BERRY && GetHealthPercentage(bankAtk) <= 25)
			{
				INCREASE_VIABILITY(4); // Counteract the negative check
			}
		}
	...
	case EFFECT_MIRROR_MOVE: // May cause issues with priority calcs?
		...
		case MOVE_COPYCAT:
			...
			COPYCAT_CHECK_LAST_MOVE:
				if (gNewBS->LastUsedMove != MOVE_NONE && gNewBS->LastUsedMove != 0xFFFF && !CheckTableForMove(gNewBS->LastUsedMove, gCopycatBannedMoves) && !MoveInMoveset(gNewBS->LastUsedMove, bankAtk)) // If you have the move, use it directly
			...
			else
			{
				...
				else if (!CheckTableForMove(predictedMove, gCopycatBannedMoves) && !MoveInMoveset(predictedMove, bankAtk))
				{
	...
	case EFFECT_DOUBLE_HIT:;
		...
		if (IS_SINGLE_BATTLE && IsClassSweeper(class) && !IsStrongestMove(move, bankAtk, bankDef)														 // This multi-hit move wouldn't be chosen normally
			&& strongestMoveEffect != EFFECT_MULTI_HIT																									 // The strongest move isn't a multi-hit move
			&& strongestMoveEffect != EFFECT_TRIPLE_KICK && strongestMoveEffect != EFFECT_DOUBLE_HIT && (MoveBlockedBySubstitute(move, bankAtk, bankDef) // Attack has to hit substitute to break it
																										 || data->atkItemEffect == ITEM_EFFECT_FLINCH))
			INCREASE_VIABILITY(3); // Move past strongest move
		break;

	case EFFECT_CONVERSION:
		if (!IsOfType(bankAtk, gBattleMoves[gBattleMons[bankAtk].moves[0]].type) && IsClassSweeper(class))
			INCREASE_STATUS_VIABILITY(1);
		break;

	case EFFECT_FLINCH_HIT:
	AI_FLINCH_CHECKS:
		if (defAbility == ABILITY_INNERFOCUS || MoveBlockedBySubstitute(move, bankAtk, bankDef) || !MoveWouldHitFirst(move, bankAtk, bankDef))
			break;
		else if (atkAbility == ABILITY_SERENEGRACE || data->defStatus1 & STATUS1_PARALYSIS || data->defStatus2 & STATUS2_INFATUATION || IsConfused(bankDef))
		{
	...
	AI_POISON_CHECKS:
		...
		{
			if (MoveInMoveset(MOVE_VENOSHOCK, bankAtk) || MoveInMoveset(MOVE_HEX, bankAtk) || MoveInMoveset(MOVE_VENOMDRENCH, bankAtk) || atkAbility == ABILITY_MERCILESS)
				INCREASE_STATUS_VIABILITY(2);
			else
	...
	case EFFECT_TRAP:
	AI_TRAP:
		if (MoveInMoveset(MOVE_RAPIDSPIN, bankDef) || IsOfType(bankDef, TYPE_GHOST) || data->defStatus2 & (STATUS2_WRAPPED))
		{
	...
	AI_FOCUS_ENERGY:
		if (atkAbility == ABILITY_SUPERLUCK || atkAbility == ABILITY_SNIPER || data->atkItemEffect == ITEM_EFFECT_SCOPE_LENS)
			INCREASE_STATUS_VIABILITY(2);
		else
			INCREASE_STATUS_VIABILITY(1);
		break;
	...
	case EFFECT_CONFUSE:
	AI_CONFUSE_CHECK:
		if (CanBeConfused(bankDef, TRUE) && data->defItemEffect != ITEM_EFFECT_CURE_CONFUSION && data->defItemEffect != ITEM_EFFECT_CURE_STATUS)
		{
			if (data->defStatus1 & STATUS1_PARALYSIS || data->defStatus2 & (STATUS2_INFATUATION) || (atkAbility == ABILITY_SERENEGRACE && FlinchingMoveInMoveset(bankAtk)))
				INCREASE_STATUS_VIABILITY(2);
			else
				INCREASE_STATUS_VIABILITY(1);
		}
		...
		if (!BadIdeaToParalyze(bankDef, bankAtk))
		{
			...
			if (IncreaseViabilityForSpeedControl(&viability, class, bankAtk, bankDef))
				break;
					 ...
					 || MoveInMoveset(MOVE_HEX, bankAtk) || FlinchingMoveInMoveset(bankAtk) || data->defStatus2 & STATUS2_INFATUATION || IsConfused(bankDef))
				INCREASE_STATUS_VIABILITY(2);
			else
				INCREASE_STATUS_VIABILITY(1);
		}
	...
	case EFFECT_SPEED_DOWN_HIT:
		if (CalcSecondaryEffectChance(bankAtk, move) >= 50)
		{
			...
			{
				if (defAbility != ABILITY_CONTRARY && defAbility != ABILITY_CLEARBODY && defAbility != ABILITY_WHITESMOKE
	...
	case EFFECT_CONFUSE_HIT:
		if (CalcSecondaryEffectChance(bankAtk, move) >= 75 && MoveWillHit(move, bankAtk, bankDef))
	...
	case EFFECT_MIMIC:
		if (MoveWouldHitFirst(move, bankAtk, bankDef))
		{
			...
			{
				if (!CanKnockOut(bankDef, bankAtk) && MoveKnocksOutXHits(gLastUsedMoves[bankDef], bankAtk, bankDef, 1))
					INCREASE_STATUS_VIABILITY(2);
		...
		{
			if (!Can2HKO(bankDef, bankAtk) && MoveKnocksOutXHits(predictedMove, bankAtk, bankDef, 1)) // Attacker can kill with mimicked move
				INCREASE_STATUS_VIABILITY(1);
			else if (MoveEffectInMoveset(EFFECT_IMPRISON, bankAtk))
				INCREASE_STATUS_VIABILITY(1);
		}
	...
	case EFFECT_LEECH_SEED:
		if (IsOfType(bankDef, TYPE_GRASS) || data->defStatus3 & STATUS3_LEECHSEED || MoveInMoveset(MOVE_RAPIDSPIN, bankDef) || defAbility == ABILITY_LIQUIDOOZE || defAbility == ABILITY_MAGICGUARD)
			break;
		else
			INCREASE_STATUS_VIABILITY(3);
		break;
	...
	case EFFECT_DISABLE:
		if (gDisableStructs[bankDef].disableTimer1 == 0 && data->defItemEffect != ITEM_EFFECT_CURE_ATTRACT)
		{
			if (MoveWouldHitFirst(move, bankAtk, bankDef))
			{
				if (gLastUsedMoves[bankDef] != MOVE_NONE && gLastUsedMoves[bankDef] != 0xFFFF)
	...
	case EFFECT_ENCORE:
		if (gDisableStructs[bankDef].encoreTimer == 0 && data->defItemEffect != ITEM_EFFECT_CURE_ATTRACT)
		{
			if (MoveWouldHitFirst(move, bankAtk, bankDef))
			{
				if (gLastUsedMoves[bankDef] != MOVE_NONE && gLastUsedMoves[bankDef] != 0xFFFF && (SPLIT(gLastUsedMoves[bankDef]) == SPLIT_STATUS || AI_SpecialTypeCalc(gLastUsedMoves[bankDef], bankDef, bankAtk) & MOVE_RESULT_DOESNT_AFFECT_FOE))
	...
	case EFFECT_SPITE:;
		...
		if (MoveWouldHitFirst(move, bankAtk, bankDef) && predictedMove != MOVE_NONE && moveSlot < 4 && gBattleMons[bankDef].pp[moveSlot] <= 4)
			INCREASE_STATUS_VIABILITY(3);
		break;
	...
	case EFFECT_THIEF:
		if (data->atkItem == ITEM_NONE && data->defItem != ITEM_NONE && CanTransferItem(SPECIES(bankDef), data->defItem) && CanTransferItem(SPECIES(bankAtk), data->defItem) && !MoveInMoveset(MOVE_ACROBATICS, bankAtk) && defAbility != ABILITY_STICKYHOLD && IsClassSweeper(class))
		{
			switch (ItemId_GetHoldEffect(ITEM(bankDef)))
			{
			case ITEM_EFFECT_CHOICE_BAND:;
				if (ITEM_QUALITY(bankDef) == QUALITY_CHOICE_SCARF || MoveSplitInMoveset(ITEM_QUALITY(bankDef), bankAtk))
					INCREASE_STATUS_VIABILITY(2);
				break;
	...
	case EFFECT_PROTECT:;
		...
		case MOVE_WIDEGUARD:
			...
			{
				if (atkAbility != ABILITY_TELEPATHY && !(AI_SpecialTypeCalc(data->partnerMove, data->bankAtkPartner, bankAtk) & MOVE_RESULT_NO_EFFECT)) // Move has effect
		...
		case MOVE_ENDURE:
			if (CanKnockOut(bankDef, bankAtk))
			{
				...
				{
					if (MoveEffectInMoveset(EFFECT_FLAIL, bankAtk) || MoveEffectInMoveset(EFFECT_ENDEAVOR, bankAtk))
						INCREASE_STATUS_VIABILITY(3);
				}
			}
			break;
		...
		case MOVE_KINGSSHIELD:
			...
			if (atkAbility == ABILITY_STANCECHANGE // Special logic for Aegislash
				&& data->atkSpecies == SPECIES_AEGISLASH_BLADE && !IsBankIncapacitated(bankDef))
			{
				if (IsClassStall(class))
	...
	case EFFECT_SPIKES:;
		...
		case MOVE_STICKYWEB:
			...
			{
				if (GetMonData(&atkParty[i], MON_DATA_HP, NULL) > 0 && GetMonData(&atkParty[i], MON_DATA_SPECIES, NULL) != SPECIES_NONE && !GetMonData(&atkParty[i], MON_DATA_IS_EGG, NULL))
				{
					for (j = 0; j < PARTY_SIZE; ++j) // Loop through target party
					{
						if (GetMonData(&defParty[j], MON_DATA_HP, NULL) > 0 && GetMonData(&defParty[j], MON_DATA_SPECIES, NULL) != SPECIES_NONE && !GetMonData(&defParty[j], MON_DATA_IS_EGG, NULL) && ItemId_GetHoldEffect(GetMonData(&defParty[j], MON_DATA_HELD_ITEM, NULL)) != ITEM_EFFECT_HEAVY_DUTY_BOOTS && j != gBattlerPartyIndexes[data->foe1] && j != gBattlerPartyIndexes[data->foe2] && CheckGroundingFromPartyData(&defParty[j]) == GROUNDED // Affected by Sticky Web
		...
		case MOVE_G_MAX_STEELSURGE_S:
			...
			{
				if (GetMonData(&defParty[i], MON_DATA_SPECIES, NULL) != SPECIES_NONE && !GetMonData(&defParty[i], MON_DATA_IS_EGG, NULL) && GetMonData(&defParty[i], MON_DATA_HP, NULL) > 0 && ItemId_GetHoldEffect(GetMonData(&defParty[i], MON_DATA_HELD_ITEM, NULL)) != ITEM_EFFECT_HEAVY_DUTY_BOOTS && i != gBattlerPartyIndexes[data->foe1] && i != gBattlerPartyIndexes[data->foe2])
				{
					IncreaseEntryHazardsViability(&viability, class, bankAtk, bankDef, move);
		...
		case MOVE_TOXICSPIKES:
			for (i = 0; i < PARTY_SIZE; ++i)
			{
				if (GetMonData(&defParty[i], MON_DATA_SPECIES, NULL) != SPECIES_NONE && !GetMonData(&defParty[i], MON_DATA_IS_EGG, NULL) && GetMonData(&defParty[i], MON_DATA_HP, NULL) > 0 && ItemId_GetHoldEffect(GetMonData(&defParty[i], MON_DATA_HELD_ITEM, NULL)) != ITEM_EFFECT_HEAVY_DUTY_BOOTS && i != gBattlerPartyIndexes[data->foe1] && i != gBattlerPartyIndexes[data->foe2] && CheckGroundingFromPartyData(&defParty[i]) == GROUNDED)
				{
	...
	case EFFECT_FORESIGHT:
		...
		{
			if (STAT_STAGE(bankDef, STAT_STAGE_EVASION) > 6 || (IsOfType(bankDef, TYPE_DARK) && (MoveTypeInMoveset(TYPE_PSYCHIC, bankAtk))))
				INCREASE_STATUS_VIABILITY(2);
			break;
		}
		else
		{
			...
			// 	break;
			else if (atkAbility == ABILITY_SCRAPPY || atkAbility == ABILITY_INNEREYE) break;
			else if (STAT_STAGE(bankDef, STAT_STAGE_EVASION) > 6 || (IsOfType(bankDef, TYPE_GHOST) && (MoveTypeInMoveset(TYPE_NORMAL, bankAtk) || MoveTypeInMoveset(TYPE_FIGHTING, bankAtk))))
				INCREASE_STATUS_VIABILITY(2);
			break;
		}
		break;
	...
	case EFFECT_SANDSTORM:
		if (IsClassDoublesSetupAttacker(class) && IsTypeZCrystal(data->atkItem, moveType) && !gNewBS->zMoveData.used[bankAtk] // Z-Crystal provides speed up
			&& !IsMegaZMoveBannedBattle() && atkAbility != ABILITY_CONTRARY)
			IncreaseTailwindViability(&viability, class, bankAtk, bankDef);

		else if (IsClassDoublesSetupAttacker(class) && atkAbility == ABILITY_SANDRUSH)
			IncreaseTailwindViability(&viability, class, bankAtk, bankDef);

		else if (atkAbility == ABILITY_SANDVEIL || atkAbility == ABILITY_SANDRUSH || atkAbility == ABILITY_SANDFORCE || atkAbility == ABILITY_SANDFORCE || atkAbility == ABILITY_OVERCOAT || atkAbility == ABILITY_MAGICGUARD || data->atkItemEffect == ITEM_EFFECT_SAFETY_GOGGLES || IsOfType(bankAtk, TYPE_ROCK) || IsOfType(bankAtk, TYPE_STEEL) || IsOfType(bankAtk, TYPE_GROUND) || MoveInMoveset(MOVE_SHOREUP, bankAtk) || MoveInMoveset(MOVE_WEATHERBALL, bankAtk) || data->atkItemEffect == ITEM_EFFECT_SMOOTH_ROCK)
		{
	...
	case EFFECT_ROLLOUT:
		if (IsClassSweeper(class) && data->atkStatus2 & STATUS2_DEFENSE_CURL)
			INCREASE_VIABILITY(8);
		break;

	case EFFECT_SWAGGER:
		if (MoveInMoveset(MOVE_FOULPLAY, bankAtk) || MoveEffectInMoveset(EFFECT_PSYCH_UP, bankAtk)) // Includes Spectral Thief
			INCREASE_STATUS_VIABILITY(2);
		else
			goto AI_CONFUSE_CHECK;
		break;
	...
	case EFFECT_FURY_CUTTER:
		if (IS_SINGLE_BATTLE && IsClassSweeper(class) && data->atkItemEffect == ITEM_EFFECT_METRONOME)
			INCREASE_VIABILITY(3); // Past strongest move
		break;

	case EFFECT_ATTRACT:
		if (IS_SINGLE_BATTLE && WillFaintFromSecondaryDamage(bankDef) && !MoveWouldHitFirst(move, bankAtk, bankDef))
			break; // Don't use if the attract will never get a chance to proc

		if (data->defStatus1 & STATUS1_ANY || IsConfused(bankDef) || IsTrapped(bankDef, TRUE))
			INCREASE_STATUS_VIABILITY(2);
		else
			INCREASE_STATUS_VIABILITY(1);
		break;
	...
	case EFFECT_SAFEGUARD:
		if (IS_SINGLE_BATTLE)
		{
			if (IsClassTeamSupport(class) && (gTerrainType != MISTY_TERRAIN || !CheckGrounding(bankAtk)))
				INCREASE_STATUS_VIABILITY(1);
		}
		else // Double Battle
		{
	...
	case EFFECT_BATON_PASS:
		...
		PIVOT_CHECK:
			if (IS_SINGLE_BATTLE)
			{
				...
				else if (gWishFutureKnock.wishCounter[bankAtk] > 0 && ShouldUseWishAromatherapy(bankAtk, bankDef, MOVE_WISH, class))
				{
					INCREASE_VIABILITY(7);
				}
			}
			else // Double Battle
			{
				...
				if (GetMonAbility(GetBankPartyData(bankAtk)) == ABILITY_INTIMIDATE && MoveSplitOnTeam(bankDef, SPLIT_PHYSICAL))
				{
	...
	case EFFECT_RAPID_SPIN:
		...
		{
			if (data->atkStatus3 & STATUS3_LEECHSEED || data->atkStatus2 & STATUS2_WRAPPED)
				INCREASE_STATUS_VIABILITY(3);
		}
		break;
	...
	case EFFECT_RAIN_DANCE:
		...
		else if (data->atkItemEffect != ITEM_EFFECT_UTILITY_UMBRELLA && (atkAbility == ABILITY_SWIFTSWIM || atkAbility == ABILITY_FORECAST || atkAbility == ABILITY_HYDRATION || atkAbility == ABILITY_RAINDISH || atkAbility == ABILITY_DRYSKIN || MoveEffectInMoveset(EFFECT_THUNDER, bankAtk) // Includes Hurricane
																		 || MoveEffectInMoveset(EFFECT_MORNING_SUN, bankDef) || MoveInMoveset(MOVE_WEATHERBALL, bankAtk) || MoveTypeInMoveset(TYPE_WATER, bankAtk) || MoveTypeInMoveset(TYPE_FIRE, bankDef) || data->atkItemEffect == ITEM_EFFECT_DAMP_ROCK))
		{
			if (IsClassDoublesTeamSupport(class))
				INCREASE_VIABILITY(17);
			else
	...
	case EFFECT_SUNNY_DAY:
		...
		else if (IsClassDoublesSetupAttacker(class) && atkAbility == ABILITY_CHLOROPHYLL)
			IncreaseTailwindViability(&viability, class, bankAtk, bankDef);

		else if (data->atkItemEffect != ITEM_EFFECT_UTILITY_UMBRELLA && (atkAbility == ABILITY_CHLOROPHYLL || atkAbility == ABILITY_FLOWERGIFT || atkAbility == ABILITY_FORECAST || atkAbility == ABILITY_LEAFGUARD || atkAbility == ABILITY_SOLARPOWER || atkAbility == ABILITY_HARVEST || MoveEffectInMoveset(EFFECT_SOLARBEAM, bankAtk) || MoveEffectInMoveset(EFFECT_MORNING_SUN, bankAtk) || MoveEffectInMoveset(EFFECT_THUNDER, bankDef) || MoveInMoveset(MOVE_WEATHERBALL, bankAtk) || MoveInMoveset(MOVE_GROWTH, bankAtk) || MoveTypeInMoveset(TYPE_FIRE, bankAtk) || MoveTypeInMoveset(TYPE_WATER, bankDef) || data->atkItemEffect == ITEM_EFFECT_HEAT_ROCK))
		{
			if (IsClassDoublesTeamSupport(class))
				INCREASE_VIABILITY(17);
			else
	...
	case EFFECT_SEMI_INVULNERABLE:
		if (IS_SINGLE_BATTLE && IsClassSweeper(class))
		{
			if (MoveWouldHitFirst(move, bankAtk, bankDef))
			{
				if (gBattleMoves[predictedMove].effect == EFFECT_EXPLOSION || gBattleMoves[predictedMove].effect == EFFECT_PROTECT)
					INCREASE_VIABILITY(3); // Past strongest move
			}
			else if (gBattleMoves[predictedMove].effect == EFFECT_SEMI_INVULNERABLE && !BATTLER_SEMI_INVULNERABLE(bankDef))
				INCREASE_VIABILITY(3); // Past strongest move
		}
		break;
	...
	case EFFECT_STOCKPILE:
		if (MoveEffectInMoveset(EFFECT_SWALLOW, bankAtk) || MoveEffectInMoveset(EFFECT_SPIT_UP, bankAtk))
			INCREASE_STATUS_VIABILITY(2);
		else
	...
	case EFFECT_HAIL:
		...
		else if (IsClassDoublesSetupAttacker(class) && IsTypeZCrystal(data->atkItem, moveType) && !gNewBS->zMoveData.used[bankAtk] // Z-Crystal provides speed up
				 && !IsMegaZMoveBannedBattle() && atkAbility != ABILITY_CONTRARY)
			IncreaseTailwindViability(&viability, class, bankAtk, bankDef);

		else if (IsClassDoublesSetupAttacker(class) && atkAbility == ABILITY_SLUSHRUSH)
			IncreaseTailwindViability(&viability, class, bankAtk, bankDef);

		else if (atkAbility == ABILITY_SNOWCLOAK || atkAbility == ABILITY_ICEBODY || atkAbility == ABILITY_FORECAST || atkAbility == ABILITY_SLUSHRUSH || atkAbility == ABILITY_MAGICGUARD || atkAbility == ABILITY_OVERCOAT || MoveInMoveset(MOVE_BLIZZARD, bankAtk) || MoveInMoveset(MOVE_AURORAVEIL, bankAtk) || MoveInMoveset(MOVE_WEATHERBALL, bankAtk) || MoveEffectInMoveset(EFFECT_MORNING_SUN, bankDef) || data->atkItemEffect == ITEM_EFFECT_ICY_ROCK)
		{
			if (IsClassDoublesTeamSupport(class))
				INCREASE_VIABILITY(17);
			else
	...
	AI_BURN_CHECKS:
		...
		{
			if (CalcMoveSplit(bankDef, predictedMove) == SPLIT_PHYSICAL && MoveKnocksOutXHits(predictedMove, bankDef, bankAtk, 1))
				INCREASE_STATUS_VIABILITY(3); // If the enemy can kill with a physical move, try burning them so they can't anymore

			else if ((IsClassDoublesUtility(class) || IsClassDoublesTeamSupport(class)) && PhysicalMoveInMoveset(bankDef))
			{
			...
			else if (MoveInMoveset(MOVE_HEX, bankAtk) || MoveInMoveset(MOVE_HEX, data->bankAtkPartner) || PhysicalMoveInMoveset(bankDef))
				INCREASE_STATUS_VIABILITY(2);
			else
				INCREASE_STATUS_VIABILITY(1);
		}
	...
	case EFFECT_FOLLOW_ME:
		if (IS_DOUBLE_BATTLE && move != MOVE_SPOTLIGHT && !IsBankIncapacitated(bankDef) && (move != MOVE_RAGEPOWDER || IsAffectedByPowder(bankDef)) // Rage Powder doesn't affect powder immunities
			&& BATTLER_ALIVE(data->bankAtkPartner))
		{
	...
	case EFFECT_CHARGE:
		...
		{
			if (GetMoveTypeSpecial(bankAtk, gBattleMons[bankAtk].moves[i]) == TYPE_ELECTRIC && SPLIT(gBattleMons[bankAtk].moves[i]) != SPLIT_STATUS)
				INCREASE_STATUS_VIABILITY(2);
		}
	...
	case EFFECT_TRICK: //+ Bestwo
		...
		{
		case ITEM_EFFECT_CHOICE_BAND:;
			if (data->atkItemQuality == QUALITY_CHOICE_SCARF || !MoveSplitInMoveset(data->atkItemQuality, bankDef)) // Target doesn't have beneficial move for choice item
				INCREASE_STATUS_VIABILITY(2);
			break;
		...
		case ITEM_EFFECT_EJECT_BUTTON:
			if (!IsRaidBattle() && IsDynamaxed(bankDef) && gNewBS->dynamaxData.timer[bankDef] > 1 && (DamagingMoveInMoveset(bankAtk) || (IS_DOUBLE_BATTLE && BATTLER_ALIVE(data->bankAtkPartner) && DamagingMoveInMoveset(data->bankAtkPartner))))
	...
	case EFFECT_SUPERPOWER:
		if (move != MOVE_HYPERSPACEHOLE && atkAbility != ABILITY_CONTRARY && data->atkItemEffect == ITEM_EFFECT_EJECT_PACK)
			goto PIVOT_CHECK;
		break;
	...
	case EFFECT_BRICK_BREAK:
		if (IsRaidBattle() && SIDE(bankAtk) == B_SIDE_PLAYER && gNewBS->dynamaxData.raidShieldsUp // Brick Break destroys 2 Raid shields
			&& (gNewBS->dynamaxData.shieldCount - gNewBS->dynamaxData.shieldsDestroyed >= 2)	  // At least two shields left
			&& !MoveBlockedBySubstitute(move, bankAtk, bankDef) && !(AI_SpecialTypeCalc(move, bankAtk, bankDef) & MOVE_RESULT_DOESNT_AFFECT_FOE))
		{
			INCREASE_VIABILITY(18); // Super important
		}
		else if (gSideStatuses[SIDE(bankDef)] & (SIDE_STATUS_REFLECT | SIDE_STATUS_LIGHTSCREEN) || gNewBS->AuroraVeilTimers[SIDE(bankDef)] > 0)
		{
			if (IsClassSweeper(class))
	...
	case EFFECT_SKILL_SWAP:
		...
		case MOVE_ENTRAINMENT:
			if (gAbilityRatings[defAbility] >= 5 || gAbilityRatings[atkAbility] <= 0)
			{
				if (defAbility != atkAbility && !IsAbilitySuppressed(bankDef))
					INCREASE_STATUS_VIABILITY(2);
			}
			break;
	...
	case EFFECT_EXTREME_EVOBOOST:;
		...
		case MOVE_CLANGOROUSSOUL:
			if (MoveKnocksOutXHits(predictedMove, bankDef, bankAtk, 1))
				break;
			else if (MoveKnocksOutXHits(predictedMove, bankDef, bankAtk, 3) && !MoveKnocksOutXHits(predictedMove, bankDef, bankAtk, 2))
		...
		if (atkAbility != ABILITY_CONTRARY)
		{
			...
			case MOVE_SHELLSMASH:
				if (data->atkItemEffect == ITEM_EFFECT_POWER_HERB)
					...
					goto AI_SPEED_PLUS;
				else if ((IsClassBatonPass(class) && STAT_STAGE(bankAtk, STAT_STAGE_SPATK) < 10) || (STAT_STAGE(bankAtk, STAT_STAGE_SPATK) < 8 && (SpecialMoveInMoveset(bankAtk))))
					goto AI_SPECIAL_ATTACK_PLUS;
				else
					goto AI_ATTACK_PLUS;
				__attribute__((fallthrough));
	...
	case EFFECT_STAT_SWAP_SPLIT:
		...
		case MOVE_GUARDSWAP: // Prevent's getting caught in an infinite loop
			if (STAT_STAGE(bankDef, STAT_STAGE_DEF) > STAT_STAGE(bankAtk, STAT_STAGE_DEF) && STAT_STAGE(bankDef, STAT_STAGE_SPDEF) >= STAT_STAGE(bankAtk, STAT_STAGE_SPDEF))
				INCREASE_STATUS_VIABILITY(1);
			else if (STAT_STAGE(bankDef, STAT_STAGE_SPDEF) > STAT_STAGE(bankAtk, STAT_STAGE_SPDEF) && STAT_STAGE(bankDef, STAT_STAGE_DEF) >= STAT_STAGE(bankAtk, STAT_STAGE_DEF))
				INCREASE_STATUS_VIABILITY(1);
			break;

		case MOVE_POWERSWAP:
			if (STAT_STAGE(bankDef, STAT_STAGE_ATK) > STAT_STAGE(bankAtk, STAT_STAGE_ATK) && STAT_STAGE(bankDef, STAT_STAGE_SPATK) >= STAT_STAGE(bankAtk, STAT_STAGE_SPATK))
				INCREASE_STATUS_VIABILITY(1);
			else if (STAT_STAGE(bankDef, STAT_STAGE_SPATK) > STAT_STAGE(bankAtk, STAT_STAGE_SPATK) && STAT_STAGE(bankDef, STAT_STAGE_ATK) >= STAT_STAGE(bankAtk, STAT_STAGE_ATK))
				INCREASE_STATUS_VIABILITY(1);
			break;
		...
		case MOVE_GUARDSPLIT:;
			...
			if ((newDefense > data->atkDefense && newSpDef >= data->atkSpDef) || (newSpDef > data->atkSpDef && newDefense >= data->atkDefense))
				INCREASE_STATUS_VIABILITY(1);
			break;
		...
		case MOVE_POWERSPLIT:;
			...
			if ((newAttack > data->atkAttack && newSpAtk >= data->atkSpAtk) || (newSpAtk > data->atkSpAtk && newAttack >= data->atkAttack))
				INCREASE_STATUS_VIABILITY(1);
			break;
		}
		break;
	...
	case EFFECT_EAT_BERRY:
		...
		case MOVE_BUGBITE:
		case MOVE_PLUCK:
			if (data->defStatus2 & STATUS2_SUBSTITUTE || defAbility == ABILITY_STICKYHOLD)
				break;
	...
	case EFFECT_FIELD_EFFECTS:
		...
		{
		case MOVE_TRICKROOM:
			if (!IsTrickRoomActive() && GetPokemonOnSideSpeedAverage(bankAtk) < GetPokemonOnSideSpeedAverage(bankDef))
			{
			...
			else if (IsTrickRoomActive() && GetPokemonOnSideSpeedAverage(bankAtk) >= GetPokemonOnSideSpeedAverage(bankDef))
			{
				if (IsClassDoublesTrickRoomer(class))
					INCREASE_VIABILITY(19);
				else
		...
		case MOVE_WONDERROOM:
			if ((PhysicalMoveInMoveset(bankDef) && data->atkDefense < data->atkSpDef) || (SpecialMoveInMoveset(bankDef) && data->atkSpDef < data->atkDefense))
				INCREASE_STATUS_VIABILITY(2);
			break;
		...
		case MOVE_IONDELUGE:
			if ((atkAbility == ABILITY_VOLTABSORB || atkAbility == ABILITY_MOTORDRIVE || atkAbility == ABILITY_LIGHTNINGROD) && GetMoveTypeSpecial(bankDef, predictedMove) == TYPE_NORMAL)
				INCREASE_STATUS_VIABILITY(2);
			break;
		}
		break;
	...
	case EFFECT_ATTACK_BLOCKERS:
		...
		case MOVE_POWDER:
			if (SPLIT(predictedMove) != SPLIT_STATUS && GetMoveTypeSpecial(bankDef, predictedMove) == TYPE_FIRE)
				INCREASE_STATUS_VIABILITY(3);
			break;
		...
		default: // Heal Block
			...
			else if (HealingMoveInMoveset(bankDef) || data->defItemEffect == ITEM_EFFECT_LEFTOVERS || (data->defItemEffect == ITEM_EFFECT_BLACK_SLUDGE && IsOfType(bankDef, TYPE_POISON)))
				INCREASE_STATUS_VIABILITY(2);
		}
		break;
	...
	case EFFECT_TYPE_CHANGES:
		...
		{
		case MOVE_SOAK:
			if (IsClassSweeper(class) && (MoveTypeInMoveset(bankAtk, TYPE_ELECTRIC) || MoveTypeInMoveset(bankAtk, TYPE_GRASS)))
	...
	case EFFECT_TOPSY_TURVY_ELECTRIFY:
		...
		{
		case MOVE_ELECTRIFY:
			if (GetMoveTypeSpecial(bankDef, predictedMove) == TYPE_NORMAL && (atkAbility == ABILITY_VOLTABSORB || atkAbility == ABILITY_MOTORDRIVE || atkAbility == ABILITY_LIGHTNINGROD))
			{
				INCREASE_STATUS_VIABILITY(3);
			}
			break;
	...
	case EFFECT_TEAM_EFFECTS:
		...
		case MOVE_MAGNETRISE:
			if (CheckGrounding(bankAtk) == GROUNDED && DamagingMoveTypeInMoveset(bankDef, TYPE_GROUND) && !(AI_SpecialTypeCalc(MOVE_EARTHQUAKE, bankDef, bankAtk) & (MOVE_RESULT_DOESNT_AFFECT_FOE | MOVE_RESULT_NOT_VERY_EFFECTIVE))) // Doesn't resist ground move
			{
	...
	case EFFECT_CAMOUFLAGE:
		if (MoveWouldHitFirst(move, bankAtk, bankDef) && moveSplit != SPLIT_STATUS && !(AI_SpecialTypeCalc(predictedMove, bankDef, bankAtk) & MOVE_RESULT_DOESNT_AFFECT_FOE))
			INCREASE_STATUS_VIABILITY(1);
		break;
	...
	case EFFECT_FLAMEBURST:
		if (IS_DOUBLE_BATTLE)
		{
			if (BATTLER_ALIVE(data->bankDefPartner) && GetHealthPercentage(data->bankDefPartner) < 12 && ABILITY(data->bankDefPartner) != ABILITY_MAGICGUARD && !IsOfType(data->bankDefPartner, TYPE_FIRE))
	...
	case EFFECT_LASTRESORT_SKYDROP:
		if (move == MOVE_SKYDROP)
		{
			if (IS_SINGLE_BATTLE)
			{
				if (IsClassSweeper(class) && IsTakingSecondaryDamage(bankDef))
					INCREASE_VIABILITY(3); // Past strongest move
			}
			else // Double Battle
			{
	...
	if (moveSplit != SPLIT_STATUS)
	{
		if (IS_SINGLE_BATTLE) // Single Battle or only 1 target left
		{
			...
			else if (!MoveEffectInMoveset(EFFECT_PROTECT, bankAtk) && !MoveWouldHitFirst(move, bankAtk, bankDef) // Attacker wouldn't hit first
			...
			else if (!MoveEffectInMoveset(EFFECT_PROTECT, bankAtk) && MoveKnocksOutXHits(predictedMove, bankDef, bankAtk, 1) // Foe can kill attacker
			...
			else if (IsStrongestMove(move, bankAtk, bankDef))
			{
				...
				if (MoveWouldHitFirst(move, bankAtk, bankDef) || !WillFaintFromSecondaryDamage(bankDef) || IsMovePredictionHealingMove(bankDef, bankAtk) || atkAbility == ABILITY_MOXIE || atkAbility == ABILITY_BEASTBOOST)
	...
	if (!(AI_THINKING_STRUCT->aiFlags & AI_SCRIPT_CHECK_GOOD_MOVE)) // Semi smart only works if the AI isn't already smart
	{
		...
		if (SPLIT(move) != SPLIT_STATUS)
		{
			...
			else if (IsStrongestMove(move, bankAtk, bankDef))
			{
				// If the attacker is slower than the target and the target is going to die
				// anyways, then do something else and let it die.
				if (MoveWouldHitFirst(move, bankAtk, bankDef) || !WillFaintFromSecondaryDamage(bankDef) || IsMovePredictionHealingMove(bankDef, bankAtk) || data->atkAbility == ABILITY_MOXIE || data->atkAbility == ABILITY_BEASTBOOST)
					INCREASE_VIABILITY(2);
			}
		}
