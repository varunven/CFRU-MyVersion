...
u8 AbilityBattleEffects(u8 caseID, u8 bank, u8 ability, u8 special, u16 moveArg)
{
	...
	if ((gBattleTypeFlags & (BATTLE_TYPE_SCRIPTED_WILD_1 | BATTLE_TYPE_GHOST)) == BATTLE_TYPE_GHOST)
	{
		...
		{						 // All of these abilities either use or make changes to
	...
	case ABILITYEFFECT_ON_SWITCHIN: // 0;
		...
		if (gStatuses3[bank] & STATUS3_SWITCH_IN_ABILITY_DONE && gLastUsedAbility != ABILITYEFFECT_SWITCH_IN_WEATHER)
			break;
		...
		case ABILITY_TRACE:;
			...
			if (IS_DOUBLE_BATTLE)
			{
				if (*GetAbilityLocation(target1) != ABILITY_NONE && gBattleMons[target1].hp != 0 && *GetAbilityLocation(target2) != ABILITY_NONE && gBattleMons[target2].hp != 0)
				{
		...
		case ABILITY_ANTICIPATION:
			for (i = 0; i < MAX_MON_MOVES; ++i)
			{
				...
				if (IS_DOUBLE_BATTLE && gBattleMons[PARTNER(FOE(bank))].hp)
				{
		...
		case ABILITY_IMPOSTER:;
			...
			if (BATTLER_ALIVE(transformBank) && !(gBattleMons[transformBank].status2 & (STATUS2_TRANSFORMED | STATUS2_SUBSTITUTE)) && !(gStatuses3[transformBank] & (STATUS3_SEMI_INVULNERABLE | STATUS3_ILLUSION)) && !IS_TRANSFORMED(bank) && !(IsRaidBattle() && transformBank == BANK_RAID_BOSS && gNewBS->dynamaxData.raidShieldsUp))
			{
				gBankAttacker = bank;
		...
		case ABILITY_SCHOOLING:
			...
			{
				if (SPECIES(bank) == SPECIES_WISHIWASHI && gBattleMons[bank].level >= 20 && gBattleMons[bank].hp > (gBattleMons[bank].maxHP / 4))
				{
				...
				else if (SPECIES(bank) == SPECIES_WISHIWASHI_S && (gBattleMons[bank].level < 20 ||
		...
		case ABILITY_SHIELDSDOWN:
			if (!(gBattleMons[bank].status2 & STATUS2_TRANSFORMED))
			{
				if (SPECIES(bank) == SPECIES_MINIOR_SHIELD && gBattleMons[bank].hp <= (gBattleMons[bank].maxHP / 2))
				{
		...
		case ABILITY_NEUTRALIZINGGAS:
			for (i = 0; i < gBattlersCount; ++i)
			{
				if (!IsAbilitySuppressed(i) // Gastro Acid has higher priority
					&& ABILITY(i) != ABILITY_NONE && !CheckTableForAbility(ABILITY(i), gNeutralizingGasBannedAbilities))
				{
		...
		switch (gLastUsedAbility)
		{				   // These abilities should always activate if they can
	...
	case ABILITYEFFECT_ENDTURN: // 1
		if (BATTLER_ALIVE(bank))
		{
			...
			case ABILITY_RAINDISH:
				if (WEATHER_HAS_EFFECT && (gBattleWeather & WEATHER_RAIN_ANY) && ITEM_EFFECT(bank) != ITEM_EFFECT_UTILITY_UMBRELLA && !BATTLER_MAX_HP(bank))
				{
			...
			case ABILITY_HYDRATION:
				if (WEATHER_HAS_EFFECT && (gBattleWeather & WEATHER_RAIN_ANY) && ITEM_EFFECT(bank) != ITEM_EFFECT_UTILITY_UMBRELLA && gBattleMons[bank].status1 & STATUS_ANY)
				{
					ClearBankStatus(bank);
					BattleScriptPushCursorAndCallback(BattleScript_AbilityCuredStatusEnd3);
					effect++;
			...
			case ABILITY_HEALER:
				if (IS_DOUBLE_BATTLE && BATTLER_ALIVE(PARTNER(bank)) && gBattleMons[PARTNER(bank)].status1 && Random() % 100 < 30)
				{
			...
			case ABILITY_EMERGENCYEXIT:
				if (!(gStatuses3[bank] & (STATUS3_SKY_DROP_ANY)) && gNewBS->turnDamageTaken[bank] > 0 && gBattleMons[bank].hp <= gBattleMons[bank].maxHP / 2 && gBattleMons[bank].hp + gNewBS->turnDamageTaken[bank] > gBattleMons[bank].maxHP / 2) // Fell from end turn damage
				{
	...
	case ABILITYEFFECT_MOVES_BLOCK: // 2
		if (gBankAttacker != bank)	// Can't block against self
		{
			...
			case ABILITY_TELEPATHY:
				if (IS_DOUBLE_BATTLE && gBankAttacker == PARTNER(bank))
					effect = 1;
				break;
			}
	...
	case ABILITYEFFECT_CONTACT:										   // After being hit by a move. Not necessarilly contact.
		...
		case ABILITY_COLORCHANGE:
			if (MOVE_HAD_EFFECT && TOOK_DAMAGE(bank) && move != MOVE_STRUGGLE && SPLIT(move) != SPLIT_STATUS && !IsOfType(bank, moveType) && BATTLER_ALIVE(bank) && gBankAttacker != bank)
			{
		...
		case ABILITY_ROUGHSKIN:
			if (MOVE_HAD_EFFECT && TOOK_DAMAGE(bank) && BATTLER_ALIVE(gBankAttacker) && gBankAttacker != bank && ABILITY(gBankAttacker) != ABILITY_MAGICGUARD && CheckContact(move, gBankAttacker))
			{
		...
		case ABILITY_EFFECTSPORE:
			if (MOVE_HAD_EFFECT && TOOK_DAMAGE(bank) && BATTLER_ALIVE(gBankAttacker) && gBankAttacker != bank && CheckContact(move, gBankAttacker) && IsAffectedByPowder(gBankAttacker) && Random() % 10 == 0)
			{
				do
				{
		...
		case ABILITY_POISONPOINT:
			if (MOVE_HAD_EFFECT && TOOK_DAMAGE(bank) && BATTLER_ALIVE(gBankAttacker) && gBankAttacker != bank && CheckContact(move, gBankAttacker) && CanBePoisoned(gBankAttacker, gBankTarget, TRUE) && umodsi(Random(), 3) == 0)
			{
		...
		case ABILITY_STATIC:
			if (MOVE_HAD_EFFECT && TOOK_DAMAGE(bank) && BATTLER_ALIVE(gBankAttacker) && gBankAttacker != bank && CheckContact(move, gBankAttacker) && CanBeParalyzed(gBankAttacker, TRUE) && umodsi(Random(), 3) == 0)
			{
		...
		case ABILITY_FLAMEBODY:
			if (MOVE_HAD_EFFECT && TOOK_DAMAGE(bank) && BATTLER_ALIVE(gBankAttacker) && gBankAttacker != bank && CheckContact(move, gBankAttacker) && CanBeBurned(gBankAttacker, TRUE) && umodsi(Random(), 3) == 0)
			{
		...
		case ABILITY_RATTLED:
			if (MOVE_HAD_EFFECT && TOOK_DAMAGE(bank) && BATTLER_ALIVE(bank) && gBankAttacker != bank && (moveType == TYPE_DARK || moveType == TYPE_BUG || moveType == TYPE_GHOST) && gBattleMons[bank].statStages[STAT_SPEED - 1] < 12)
			{
				gBattleScripting.statChanger = STAT_SPEED | INCREASE_1;
				BattleScriptPushCursor();
				gBattlescriptCurrInstr = BattleScript_TargetAbilityStatRaise;
		...
		case ABILITY_WEAKARMOR:
			if (MOVE_HAD_EFFECT && TOOK_DAMAGE(bank) && CalcMoveSplit(gBankAttacker, gCurrentMove) == SPLIT_PHYSICAL && BATTLER_ALIVE(bank) && gBankAttacker != bank && (gBattleMons[bank].statStages[STAT_SPEED - 1] < 12 || gBattleMons[bank].statStages[STAT_DEF - 1] > 0))
			{
				BattleScriptPushCursor();
		...
		case ABILITY_CURSEDBODY:
			if (MOVE_HAD_EFFECT && TOOK_DAMAGE(bank) && BATTLER_ALIVE(gBankAttacker) && gBankAttacker != bank && ABILITY(gBankAttacker) != ABILITY_AROMAVEIL && !(IS_DOUBLE_BATTLE && ABILITY(PARTNER(gBankAttacker)) == ABILITY_AROMAVEIL) && umodsi(Random(), 3) == 0)
			{
				BattleScriptPushCursor();
		...
		case ABILITY_MUMMY:
			if (MOVE_HAD_EFFECT && TOOK_DAMAGE(bank) && BATTLER_ALIVE(gBankAttacker) && gBankAttacker != bank && CheckContact(move, gBankAttacker))
			{
		...
		case ABILITY_WANDERING_SPIRIT:
			if (MOVE_HAD_EFFECT && TOOK_DAMAGE(bank) && (BATTLER_ALIVE(gBankAttacker) || BATTLER_ALIVE(gBankTarget)) && gBankAttacker != bank && CheckContact(move, gBankAttacker))
			{
				BattleScriptPushCursor();
		...
		case ABILITY_ANGERPOINT:
			if (MOVE_HAD_EFFECT && TOOK_DAMAGE(bank) && gCritMultiplier > BASE_CRIT_MULTIPLIER && BATTLER_ALIVE(bank) && gBattleMons[bank].statStages[STAT_ATK - 1] < 12)
			{
		...
		case ABILITY_AFTERMATH:
			if (MOVE_HAD_EFFECT && TOOK_DAMAGE(bank) && BATTLER_ALIVE(gBankAttacker) && gBankAttacker != bank && ABILITY(gBankAttacker) != ABILITY_MAGICGUARD && CheckContact(move, gBankAttacker) && !BATTLER_ALIVE(bank) && !ABILITY_PRESENT(ABILITY_DAMP))
			{
		...
		case ABILITY_BERSERK:
			if (MOVE_HAD_EFFECT && TOOK_DAMAGE(bank) && BATTLER_ALIVE(bank) && gBattleMons[bank].hp < gBattleMons[bank].maxHP / 2 && gBattleMons[bank].hp + gHpDealt > gBattleMons[bank].maxHP / 2 // Hp fell below half
		...
		case ABILITY_GOOEY:
			if (MOVE_HAD_EFFECT && TOOK_DAMAGE(bank) && CheckContact(move, gBankAttacker) && BATTLER_ALIVE(gBankAttacker) && gBankAttacker != bank && (STAT_CAN_FALL(gBankAttacker, STAT_SPD) || ABILITY(gBankAttacker) == ABILITY_MIRRORARMOR))
			{
		...
		case ABILITY_INNARDSOUT:
			if (MOVE_HAD_EFFECT && TOOK_DAMAGE(bank) && BATTLER_ALIVE(gBankAttacker) && gBankAttacker != bank && ABILITY(gBankAttacker) != ABILITY_MAGICGUARD && !BATTLER_ALIVE(bank))
			{
		...
		case ABILITY_STAMINA:
			if (MOVE_HAD_EFFECT && TOOK_DAMAGE(bank) && BATTLER_ALIVE(bank) && gBankAttacker != bank && gBattleMons[bank].statStages[STAT_DEF - 1] < 12)
			{
		...
		case ABILITY_WATERCOMPACTION:
			if (MOVE_HAD_EFFECT && TOOK_DAMAGE(bank) && BATTLER_ALIVE(bank) && gBankAttacker != bank && moveType == TYPE_WATER && gBattleMons[bank].statStages[STAT_DEF - 1] < 12)
			{
		...
		case ABILITY_STEAMENGINE:
			if (MOVE_HAD_EFFECT && TOOK_DAMAGE(bank) && BATTLER_ALIVE(bank) && gBankAttacker != bank && (moveType == TYPE_WATER || moveType == TYPE_FIRE) && STAT_STAGE(bank, STAT_SPEED) < STAT_STAGE_MAX)
			{
		...
		case ABILITY_ILLUSION:
			if (MOVE_HAD_EFFECT && TOOK_DAMAGE(bank) && gStatuses3[bank] & STATUS3_ILLUSION)
			{
		...
		case ABILITY_COTTONDOWN:
			if (MOVE_HAD_EFFECT && TOOK_DAMAGE(bank))
			{
			}
			break;

		case ABILITY_SANDSPIT:
			if (MOVE_HAD_EFFECT && TOOK_DAMAGE(bank) && gBankAttacker != bank && !(gBattleWeather & (WEATHER_SANDSTORM_ANY | WEATHER_PRIMAL_ANY | WEATHER_CIRCUS)))
			{
				effect = ActivateWeatherAbility(WEATHER_SANDSTORM_PERMANENT | WEATHER_SANDSTORM_TEMPORARY,
		...
		case ABILITY_PERISH_BODY:
			if (MOVE_HAD_EFFECT && TOOK_DAMAGE(bank) && gBankAttacker != bank && CheckContact(move, gBankAttacker))
			{
				BattleScriptPushCursor();
	...
	case ABILITYEFFECT_SYNCHRONIZE: // 7
		if (gLastUsedAbility == ABILITY_SYNCHRONIZE && gNewBS->synchronizeTarget[bank] > 0)
		{
...
static void SpriteCb_AbilityPopUp(struct Sprite *sprite)
{
	if (!sprite->tHide) // Show
	{
		...
		if ((!sprite->tRightToLeft && (sprite->pos1.x -= 4) <= sprite->tOriginalX) || (sprite->tRightToLeft && (sprite->pos1.x += 4) >= sprite->tOriginalX))
		{
	...
	else // Hide
	{
		if (sprite->tFrames == 0)
		{
			if ((!sprite->tRightToLeft && (sprite->pos1.x += 4) >= sprite->tOriginalX + ABILITY_POP_UP_POS_X_SLIDE + 16) || (sprite->tRightToLeft && (sprite->pos1.x -= 4) <= sprite->tOriginalX - ABILITY_POP_UP_POS_X_SLIDE - 16))
			{
...
static void Task_FreeAbilityPopUpGfx(u8 taskId)
{
	if (!gSprites[gTasks[taskId].tSpriteId1].inUse && !gSprites[gTasks[taskId].tSpriteId2].inUse && !gNewBS->activeAbilityPopUps)
	{
