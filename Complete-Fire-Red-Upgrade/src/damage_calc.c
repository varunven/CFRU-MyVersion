...
void atk04_critcalc(void)
{
	...
	for (u32 bankDef = 0; bankDef < gBattlersCount; ++bankDef)
	{
		...
		else if (!BATTLER_ALIVE(bankDef) || bankDef == gBankAttacker || (bankDef == PARTNER(gBankAttacker) && !(gBattleMoves[gCurrentMove].target & MOVE_TARGET_ALL)) || gNewBS->ResultFlags[bankDef] & MOVE_RESULT_NO_EFFECT || gNewBS->noResultString[bankDef])
			continue; // Don't bother with this target

		u8 defAbility = ABILITY(bankDef);

		if (defAbility == ABILITY_BATTLEARMOR || defAbility == ABILITY_SHELLARMOR || CantScoreACrit(gBankAttacker, NULL) || gBattleTypeFlags & (BATTLE_TYPE_OLD_MAN | BATTLE_TYPE_OAK_TUTORIAL | BATTLE_TYPE_POKE_DUDE) || gNewBS->LuckyChantTimers[SIDE(bankDef)])
		{
			confirmedCrit = FALSE;
		}
		else if ((atkAbility == ABILITY_MERCILESS && (gBattleMons[bankDef].status1 & STATUS_PSN_ANY)) || IsLaserFocused(gBankAttacker) || CheckTableForMove(gCurrentMove, gAlwaysCriticalMoves))
		{
			confirmedCrit = TRUE;
		}
		else
		{
			critChance = 2 * ((gBattleMons[gBankAttacker].status2 & STATUS2_FOCUS_ENERGY) != 0) + gNewBS->chiStrikeCritBoosts[gBankAttacker] + (CheckTableForMove(gCurrentMove, gHighCriticalChanceMoves)) + (atkEffect == ITEM_EFFECT_SCOPE_LENS) + (atkAbility == ABILITY_SUPERLUCK)
...
static u8 CalcPossibleCritChance(u8 bankAtk, u8 bankDef, u16 move, struct Pokemon *monAtk, struct Pokemon *monDef)
{
	...
	if (defAbility == ABILITY_BATTLEARMOR || defAbility == ABILITY_SHELLARMOR || CantScoreACrit(bankAtk, monAtk) || gBattleTypeFlags & (BATTLE_TYPE_OLD_MAN | BATTLE_TYPE_OAK_TUTORIAL) || gNewBS->LuckyChantTimers[SIDE(bankDef)])
		return FALSE;

	else if ((atkAbility == ABILITY_MERCILESS && (defStatus1 & STATUS_PSN_ANY)) || (IsLaserFocused(bankAtk) && monAtk == NULL) || CheckTableForMove(move, gAlwaysCriticalMoves))
		return TRUE;

	else
	{
		critChance = 2 * ((atkStatus2 & STATUS2_FOCUS_ENERGY) != 0) + gNewBS->chiStrikeCritBoosts[bankAtk] + (CheckTableForMove(move, gHighCriticalChanceMoves)) + (atkEffect == ITEM_EFFECT_SCOPE_LENS) + (atkAbility == ABILITY_SUPERLUCK)
#ifdef SPECIES_CHANSEY
...
void atk05_damagecalc(void)
{
	...
	else if (IS_DOUBLE_BATTLE && gBattleMoves[gCurrentMove].target & (MOVE_TARGET_BOTH | MOVE_TARGET_ALL))
	{
		...
		for (u32 bankDef = 0; bankDef < gBattlersCount; ++bankDef)
		{
			if (!BATTLER_ALIVE(bankDef) || bankDef == gBankAttacker || (bankDef == PARTNER(gBankAttacker) && !(gBattleMoves[gCurrentMove].target & MOVE_TARGET_ALL)) || gNewBS->ResultFlags[bankDef] & MOVE_RESULT_NO_EFFECT || gNewBS->noResultString[bankDef])
				continue; // Don't bother with this target
...
u32 AI_CalcDmg(const u8 bankAtk, const u8 bankDef, const u16 move, struct DamageCalc *damageData)
{
	...
	if (gBattleMoves[move].effect != EFFECT_PAIN_SPLIT && (SPLIT(move) == SPLIT_STATUS || resultFlags & MOVE_RESULT_NO_EFFECT))
		return 0;
	...
	if (gBattleMoves[move].effect == EFFECT_FALSE_SWIPE || (BATTLER_MAX_HP(bankDef) && ABILITY(bankDef) == ABILITY_STURDY && NO_MOLD_BREAKERS(ABILITY(bankAtk), move)) || (BATTLER_MAX_HP(bankDef) && IsBankHoldingFocusSash(bankDef)))
...
u32 AI_CalcPartyDmg(u8 bankAtk, u8 bankDef, u16 move, struct Pokemon *monAtk, struct DamageCalc *damageData)
{
	...
	// Multi hit moves skip these checks
	if (gBattleMoves[move].effect == EFFECT_FALSE_SWIPE || (BATTLER_MAX_HP(bankDef) && ABILITY(bankDef) == ABILITY_STURDY && NO_MOLD_BREAKERS(GetMonAbility(monAtk), move)) || (BATTLER_MAX_HP(bankDef) && IsBankHoldingFocusSash(bankDef)))
		damage = MathMin(damage, gBattleMons[bankDef].hp - 1);
...
u32 AI_CalcMonDefDmg(u8 bankAtk, u8 bankDef, u16 move, struct Pokemon *monDef, struct DamageCalc *damageData)
{
	...
	// Multi hit moves skip these checks
	if (gBattleMoves[move].effect == EFFECT_FALSE_SWIPE || (monDef->hp == monDef->maxHP && GetMonAbility(monDef) == ABILITY_STURDY && NO_MOLD_BREAKERS(ABILITY(bankAtk), move)) || (monDef->hp == monDef->maxHP && IsBankHoldingFocusSash(bankDef)))
...
void atk06_typecalc(void)
{
	...
	if (gCurrentMove != MOVE_STRUGGLE)
	{
		for (u32 bankDef = 0; bankDef < gBattlersCount; ++bankDef)
		{
			...
			else if (!BATTLER_ALIVE(bankDef) || bankDef == gBankAttacker || (bankDef == PARTNER(gBankAttacker) && !(gBattleMoves[gCurrentMove].target & MOVE_TARGET_ALL)) || gNewBS->noResultString[bankDef])
				continue;
			...
			if (defAbility == ABILITY_WONDERGUARD
				...
				&& (!(gNewBS->ResultFlags[bankDef] & MOVE_RESULT_SUPER_EFFECTIVE) || ((gNewBS->ResultFlags[bankDef] & (MOVE_RESULT_SUPER_EFFECTIVE | MOVE_RESULT_NOT_VERY_EFFECTIVE)) == (MOVE_RESULT_SUPER_EFFECTIVE | MOVE_RESULT_NOT_VERY_EFFECTIVE))) && SPLIT(gCurrentMove) != SPLIT_STATUS)
			{
				gLastUsedAbility = defAbility;
...
void atk4A_typecalc2(void)
{
	...
	if (defAbility == ABILITY_WONDERGUARD
		//&& AttacksThisTurn(gBankAttacker, gCurrentMove) == 2
		&& (!(gMoveResultFlags & MOVE_RESULT_SUPER_EFFECTIVE) || ((gMoveResultFlags & (MOVE_RESULT_SUPER_EFFECTIVE | MOVE_RESULT_NOT_VERY_EFFECTIVE)) == (MOVE_RESULT_SUPER_EFFECTIVE | MOVE_RESULT_NOT_VERY_EFFECTIVE))) && gBattleMoves[gCurrentMove].power)
	{
		gLastUsedAbility = defAbility;
...
u8 TypeCalc(u16 move, u8 bankAtk, u8 bankDef, struct Pokemon *monAtk, bool8 CheckParty)
{
	...
	// Check Special Ground Immunities
	if (moveType == TYPE_GROUND && !CheckGrounding(bankDef) && ((defAbility == ABILITY_LEVITATE && NO_MOLD_BREAKERS(atkAbility, move)) || defEffect == ITEM_EFFECT_AIR_BALLOON || (gStatuses3[bankDef] & (STATUS3_LEVITATING | STATUS3_TELEKINESIS))) && move != MOVE_THOUSANDARROWS)
	{
	...
	// Wonder Guard Check
	if (defAbility == ABILITY_WONDERGUARD && NO_MOLD_BREAKERS(atkAbility, move) && !(flags & MOVE_RESULT_MISSED) && AttacksThisTurn(bankAtk, move) == 2 && (!(flags & MOVE_RESULT_SUPER_EFFECTIVE) || ((flags & (MOVE_RESULT_SUPER_EFFECTIVE | MOVE_RESULT_NOT_VERY_EFFECTIVE)) == (MOVE_RESULT_SUPER_EFFECTIVE | MOVE_RESULT_NOT_VERY_EFFECTIVE))) && gBattleMoves[move].power && SPLIT(move) != SPLIT_STATUS)
	{
...
u8 AI_TypeCalc(u16 move, u8 bankAtk, struct Pokemon *monDef)
{
	...
	//  	flags = MOVE_RESULT_MISSED | MOVE_RESULT_DOESNT_AFFECT_FOE;
	//  }
	if (moveType == TYPE_GROUND && !CheckGroundingFromPartyData(monDef) && ((defAbility == ABILITY_LEVITATE && NO_MOLD_BREAKERS(atkAbility, move))) && move != MOVE_THOUSANDARROWS)
	{
		flags = MOVE_RESULT_MISSED | MOVE_RESULT_DOESNT_AFFECT_FOE;
	}
	else if (CheckTableForMove(move, gPowderMoves) && (defAbility == ABILITY_OVERCOAT || defEffect == ITEM_EFFECT_SAFETY_GOGGLES || defType1 == TYPE_GRASS || defType2 == TYPE_GRASS))
	{
		flags |= (MOVE_RESULT_MISSED | MOVE_RESULT_DOESNT_AFFECT_FOE);
	}
	...
	if (defAbility == ABILITY_WONDERGUARD && (!(flags & MOVE_RESULT_SUPER_EFFECTIVE) || ((flags & (MOVE_RESULT_SUPER_EFFECTIVE | MOVE_RESULT_NOT_VERY_EFFECTIVE)) == (MOVE_RESULT_SUPER_EFFECTIVE | MOVE_RESULT_NOT_VERY_EFFECTIVE))) && gBattleMoves[move].power)
	{
		flags |= MOVE_RESULT_DOESNT_AFFECT_FOE;
	}
...
u8 VisualTypeCalc(u16 move, u8 bankAtk, u8 bankDef)
{
	...
	// Check Special Ground Immunities
	if (moveType == TYPE_GROUND && !NonInvasiveCheckGrounding(bankDef) && ((defAbility == ABILITY_LEVITATE && NO_MOLD_BREAKERS(atkAbility, move)) || defEffect == ITEM_EFFECT_AIR_BALLOON || (gStatuses3[bankDef] & (STATUS3_LEVITATING | STATUS3_TELEKINESIS))) && move != MOVE_THOUSANDARROWS)
	{
		flags |= (MOVE_RESULT_MISSED | MOVE_RESULT_DOESNT_AFFECT_FOE);
	}
	else if (CheckTableForMove(move, gPowderMoves) && !IsAffectedByPowderByDetails(defType1, defType2, defType3, defAbility, defEffect))
	...
	// Wonder Guard Check
	if (defAbility == ABILITY_WONDERGUARD && NO_MOLD_BREAKERS(atkAbility, move) && !(flags & MOVE_RESULT_MISSED) && (!(flags & MOVE_RESULT_SUPER_EFFECTIVE) || ((flags & (MOVE_RESULT_SUPER_EFFECTIVE | MOVE_RESULT_NOT_VERY_EFFECTIVE)) == (MOVE_RESULT_SUPER_EFFECTIVE | MOVE_RESULT_NOT_VERY_EFFECTIVE))) && gBattleMoves[move].power && SPLIT(move) != SPLIT_STATUS)
		flags |= MOVE_RESULT_MISSED;
...
static void ModulateDmgByType(u8 multiplier, const u16 move, const u8 moveType, const u8 defType, const u8 bankDef, u8 atkAbility, u8 *flags, struct Pokemon *monDef, bool8 checkMonDef)
{
	...
	{
		if ((defType == TYPE_GHOST && (moveType == TYPE_NORMAL || moveType == TYPE_FIGHTING)) && (gBattleMons[bankDef].status2 & STATUS2_FORESIGHT || atkAbility == ABILITY_SCRAPPY))
	...
	if (checkMonDef)
	{
		...
		else if (multiplier == TYPE_MUL_NO_EFFECT && moveType == TYPE_GROUND && (CheckGroundingFromPartyData(monDef) || move == MOVE_THOUSANDARROWS))
			multiplier = TYPE_MUL_NORMAL;
	}
	else
	{
		if (multiplier == TYPE_MUL_NO_EFFECT && ITEM_EFFECT(bankDef) == ITEM_EFFECT_RING_TARGET)
			multiplier = TYPE_MUL_NORMAL;
		else if (multiplier == TYPE_MUL_NO_EFFECT && moveType == TYPE_GROUND && (CheckGrounding(bankDef) || move == MOVE_THOUSANDARROWS))
			multiplier = TYPE_MUL_NORMAL;
	}
...
u8 GetMoveTypeSpecialPostAbility(u16 move, u8 atkAbility, bool8 zMoveActive)
{
	...
	static bool8 AbilityCanChangeTypeAndBoost(u16 move, u8 atkAbility, u8 electrifyTimer, bool8 checkIonDeluge, bool8 zMoveActive)
	{
		...
		if (electrifyTimer > 0 || IsAnyMaxMove(move) || CheckTableForMove(move, gTypeChangeExceptionMoves))
			return FALSE;
	...
	u8 GetExceptionMoveType(u8 bankAtk, u16 move)
	{
		...
		case MOVE_HIDDENPOWER:
			moveType = ((gBattleMons[bankAtk].hpIV & 1)) | ((gBattleMons[bankAtk].attackIV & 1) << 1) | ((gBattleMons[bankAtk].defenseIV & 1) << 2) | ((gBattleMons[bankAtk].speedIV & 1) << 3) | ((gBattleMons[bankAtk].spAttackIV & 1) << 4) | ((gBattleMons[bankAtk].spDefenseIV & 1) << 5);
	...
	void AdjustDamage(bool8 checkFalseSwipe)
	{
		...
		for (u32 bankDef = 0; bankDef < gBattlersCount; ++bankDef)
		{
			...
			if ((gNewBS->zMoveData.active || IsAnyMaxMove(gCurrentMove)) && !IsDynamaxed(bankDef) && ProtectsAgainstZMoves(gCurrentMove, gBankAttacker, bankDef))
			...
			if (checkFalseSwipe)
			{
				if (gBattleMoves[gCurrentMove].effect != EFFECT_FALSE_SWIPE && !gProtectStructs[bankDef].endured && !gProtectStructs[bankDef].enduredSturdy && !gSpecialStatuses[bankDef].focusBanded)
					goto END;
			}
			else
			{
				if (!gProtectStructs[bankDef].endured && !gProtectStructs[bankDef].enduredSturdy && !gSpecialStatuses[bankDef].focusBanded)
					goto END;
			}
	...
	static s32 CalculateBaseDamage(struct DamageCalc * data)
	{
		...
		case ABILITY_FLASHFIRE:
			// 1.5x Boost
			if (!useMonAtk && data->moveType == TYPE_FIRE && (gBattleResources->flags->flags[bankAtk] & RESOURCE_FLAG_FLASH_FIRE))
				spAttack = (spAttack * 15) / 10;
			break;
...
// 0.75x Decrement
#ifdef PORTAL_POWER
			if ((useMonAtk && !CheckContactByMon(move, data->monAtk)) || (!useMonAtk && !CheckContact(move, bankAtk)))
			{
		...
		case ITEM_EFFECT_EVIOLITE:
			if ((useMonDef && CanEvolve(data->monDef)) || (!useMonDef && CanEvolve(GetBankPartyData(bankDef))))
			{
				data->defense = (data->defense * 15) / 10;
...
#ifdef BADGE_BOOSTS
		// 1.1x Boosts
		if (!(gBattleTypeFlags & (BATTLE_TYPE_LINK | BATTLE_TYPE_TRAINER_TOWER | BATTLE_TYPE_FRONTIER | BATTLE_TYPE_EREADER_TRAINER)) && gBattleTypeFlags & BATTLE_TYPE_TRAINER && SIDE(bankAtk) == B_SIDE_PLAYER && gTrainerBattleOpponent_A != TRAINER_SECRET_BASE)
		{
		...
		if (WEATHER_HAS_EFFECT && (gBattleWeather & WEATHER_SANDSTORM_ANY) && ((!useMonDef && IsOfType(bankDef, TYPE_ROCK)) || (useMonDef && IsMonOfType(data->monDef, TYPE_ROCK))))
		...
		{
		case MOVE_ASSURANCE:
			if (!(data->specialFlags & (FLAG_CHECKING_FROM_MENU | FLAG_AI_CALC)) && (gProtectStructs[bankDef].physicalDmg || gProtectStructs[bankDef].specialDmg))
				damage *= 2;
			break;
		...
		case MOVE_KNOCKOFF:
			if ((!useMonDef && CanKnockOffItem(bankDef)) || (useMonDef && CanKnockOffMonItem(data->monDef, SIDE(bankDef))))
				damage = (damage * 15) / 10;
			break;

		case MOVE_SOLARBEAM:
		case MOVE_SOLARBLADE:
			if (WEATHER_HAS_EFFECT && data->atkItemEffect != ITEM_EFFECT_UTILITY_UMBRELLA && gBattleWeather & (WEATHER_RAIN_ANY | WEATHER_SANDSTORM_ANY | WEATHER_HAIL_ANY | WEATHER_FOG_ANY | WEATHER_AIR_CURRENT_PRIMAL))
		...
		// Aura Abilities
		if ((data->moveType == TYPE_DARK && (ABILITY_PRESENT(ABILITY_DARKAURA) || data->atkAbility == ABILITY_DARKAURA || data->defAbility == ABILITY_DARKAURA)) // Check all because may be party mon
			|| (data->moveType == TYPE_FAIRY && (ABILITY_PRESENT(ABILITY_FAIRYAURA) || data->atkAbility == ABILITY_FAIRYAURA || data->defAbility == ABILITY_FAIRYAURA)))
		{
		...
		case ITEM_EFFECT_WEAKNESS_BERRY:
			...
			{
				if ((data->resultFlags & MOVE_RESULT_SUPER_EFFECTIVE && data->defItemQuality == data->moveType) || (data->defItemQuality == TYPE_NORMAL && data->moveType == TYPE_NORMAL)) // Chilan Berry
				{
		...
		if (gBattleMoves[move].effect != EFFECT_BRICK_BREAK) // Shatters the screens
		{
			...
			{
			case SPLIT_PHYSICAL:
				if ((data->defSideStatus & SIDE_STATUS_REFLECT || gNewBS->AuroraVeilTimers[SIDE(bankDef)]) && gCritMultiplier <= BASE_CRIT_MULTIPLIER && data->atkAbility != ABILITY_INFILTRATOR)
				{
			...
			case SPLIT_SPECIAL:
				if ((data->defSideStatus & SIDE_STATUS_LIGHTSCREEN || gNewBS->AuroraVeilTimers[SIDE(bankDef)]) && gCritMultiplier <= BASE_CRIT_MULTIPLIER && data->atkAbility != ABILITY_INFILTRATOR)
				{
					if (IS_DOUBLE_BATTLE && CountAliveMonsInBattle(BATTLE_ALIVE_DEF_SIDE, bankAtk, bankDef) >= 2)
						damage = (damage * 2) / 3;
					else
		...
		// Burn Cut
		if (data->moveSplit == SPLIT_PHYSICAL && (data->atkStatus1 & STATUS_BURN) && data->atkAbility != ABILITY_GUTS && move != MOVE_FACADE)
			damage /= 2;
	...
	static u16 GetBasePower(struct DamageCalc * data)
	{
		...
		case MOVE_AVALANCHE:
		case MOVE_REVENGE:
			if (!(data->specialFlags & (FLAG_CHECKING_FROM_MENU | FLAG_AI_CALC)) && !useMonAtk && ((gProtectStructs[bankAtk].physicalDmg && gProtectStructs[bankAtk].physicalBank == bankDef) || (gProtectStructs[bankAtk].specialDmg && gProtectStructs[bankAtk].specialBank == bankDef)))
				power *= 2;
			break;

		case MOVE_BRINE:
			if (!(data->specialFlags & FLAG_IGNORE_TARGET) && data->defHP < data->defMaxHP / 2)
				power *= 2;
			break;
		...
		case MOVE_FUSIONBOLT:
			if (!(data->specialFlags & (FLAG_CHECKING_FROM_MENU | FLAG_AI_CALC)) && !useMonAtk && gNewBS->fusionFlareUsedPrior && !IsFirstAttacker(bankAtk))
				power *= 2;
			break;

		case MOVE_FUSIONFLARE:
			if (!(data->specialFlags & (FLAG_CHECKING_FROM_MENU | FLAG_AI_CALC)) && !useMonAtk && gNewBS->fusionBoltUsedPrior && !IsFirstAttacker(bankAtk))
				power *= 2;
			break;

		case MOVE_HEX:
			if (!(data->specialFlags & FLAG_IGNORE_TARGET) && data->defStatus1 & STATUS_ANY)
				power *= 2;
			break;

		case MOVE_PAYBACK:
			if (!(data->specialFlags & (FLAG_IGNORE_TARGET | FLAG_CHECKING_FROM_MENU)) && !useMonAtk && !useMonDef && BankMovedBefore(bankDef, bankAtk))
				power *= 2;
			break;
		...
		case MOVE_ROUND:
			if (!(data->specialFlags & (FLAG_CHECKING_FROM_MENU | FLAG_AI_CALC)) && !useMonAtk && gNewBS->roundUsed)
				power *= 2;
			break;

		case MOVE_SMELLINGSALTS:
			if (!(data->specialFlags & FLAG_IGNORE_TARGET) && data->defStatus1 & STATUS_PARALYSIS)
				power *= 2;
			break;
		...
		case MOVE_VENOSHOCK:
			if (!(data->specialFlags & FLAG_IGNORE_TARGET) && data->defStatus1 & STATUS_PSN_ANY)
				power *= 2;
			break;

		case MOVE_WAKEUPSLAP:
			if (!(data->specialFlags & FLAG_IGNORE_TARGET) && data->defStatus1 & STATUS_SLEEP)
				power *= 2;
			break;
		...
		case MOVE_WHIRLPOOL:
			if (!(data->specialFlags & FLAG_IGNORE_TARGET) && data->defStatus3 & STATUS3_UNDERWATER)
				power *= 2;
			break;

		case MOVE_GUST:
		case MOVE_TWISTER:
			if (!(data->specialFlags & FLAG_IGNORE_TARGET) && data->defStatus3 & (STATUS3_IN_AIR | STATUS3_SKY_DROP_ATTACKER | STATUS3_SKY_DROP_TARGET))
				power *= 2;
			break;
		...
		case MOVE_TRUMPCARD:;
			...
			if (decrement && pp > 0) // PP will be decremented at runtime so imitate this
			{
				...
				if (pp > 0 && data->defAbility == ABILITY_PRESSURE && !(data->specialFlags & FLAG_IGNORE_TARGET))
		...
		case MOVE_PURSUIT:
			...
			{
				if (!useMonAtk && !useMonDef && IsPredictedToSwitch(bankDef, bankAtk))
					power *= 2;
			}
		...
		case MOVE_RETURN:
			if ((gBattleTypeFlags & (BATTLE_TYPE_LINK | BATTLE_TYPE_TRAINER_TOWER | BATTLE_TYPE_FRONTIER | BATTLE_TYPE_EREADER_TRAINER)) || IsFrontierRaidBattle())
				power = (10 * 255) / 25;
			else if (useMonAtk)
	...
	static u16 AdjustBasePower(struct DamageCalc * data, u16 power)
	{
		...
		case ABILITY_SANDFORCE:
			// 1.3x Boost
			if (gBattleWeather & WEATHER_SANDSTORM_ANY && (data->moveType == TYPE_ROCK || data->moveType == TYPE_GROUND || data->moveType == TYPE_STEEL))
				power = (power * 13) / 10;
			break;
		...
		case ABILITY_TOUGHCLAWS:
			// 1.3x Boost
			if (gBattleMoves[move].flags & FLAG_MAKES_CONTACT && data->atkItemEffect != ITEM_EFFECT_PROTECTIVE_PADS)
				power = (power * 13) / 10;
			break;
		...
		case ITEM_EFFECT_GRISEOUS_ORB:
			// 1.2x Boost
			if ((data->atkSpecies == SPECIES_GIRATINA || data->atkSpecies == SPECIES_GIRATINA_ORIGIN) && (data->moveType == TYPE_GHOST || data->moveType == TYPE_DRAGON))
				power = (power * 12) / 10;
			break;
#endif
		...
		case ITEM_EFFECT_SOUL_DEW:
			// 1.2x Boost
			if ((data->atkSpecies == SPECIES_LATIOS || data->atkSpecies == SPECIES_LATIAS) && (data->moveType == TYPE_PSYCHIC || data->moveType == TYPE_DRAGON))
				power = (power * 12) / 10;
			break;
#endif
...
#define TERRAIN_BOOST 13 // 1.3x in Gen 8
		...
		case GRASSY_TERRAIN:
			...
			if ((move == MOVE_MAGNITUDE || move == MOVE_EARTHQUAKE || move == MOVE_BULLDOZE) && !(data->defStatus3 & STATUS3_SEMI_INVULNERABLE))
				power /= 2;
			break;
