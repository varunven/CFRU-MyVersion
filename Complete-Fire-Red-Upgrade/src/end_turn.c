...
u8 TurnBasedEffects(void)
{
	...
	while (gBattleStruct->turnEffectsTracker <= TURNBASED_MAX_CASE)
	{
		...
		case ET_Block_A:
			...
			{
			case ET_Sea_Of_Fire:
				if (BATTLER_ALIVE(gActiveBattler) && BankSideHasSeaOfFire(gActiveBattler) && ABILITY(gActiveBattler) != ABILITY_MAGICGUARD && !IsOfType(gActiveBattler, TYPE_FIRE))
				{
			...
			case ET_G_Max_VineLash:
				if (BATTLER_ALIVE(gActiveBattler) && BankSideHasGMaxVineLash(gActiveBattler) && ABILITY(gActiveBattler) != ABILITY_MAGICGUARD && !IsOfType(gActiveBattler, TYPE_GRASS))
				{
			...
			case ET_G_Max_Wildfire:
				if (BATTLER_ALIVE(gActiveBattler) && BankSideHasGMaxWildfire(gActiveBattler) && ABILITY(gActiveBattler) != ABILITY_MAGICGUARD && !IsOfType(gActiveBattler, TYPE_FIRE))
				{
			...
			case ET_G_Max_Cannonade:
				if (BATTLER_ALIVE(gActiveBattler) && BankSideHasGMaxCannonade(gActiveBattler) && ABILITY(gActiveBattler) != ABILITY_MAGICGUARD && !IsOfType(gActiveBattler, TYPE_WATER))
				{
			...
			case ET_G_Max_Volcalith:
				if (BATTLER_ALIVE(gActiveBattler) && BankSideHasGMaxVolcalith(gActiveBattler) && ABILITY(gActiveBattler) != ABILITY_MAGICGUARD)
				{
			...
			case ET_Grassy_Terrain:
				if (gTerrainType == GRASSY_TERRAIN && CheckGrounding(gActiveBattler) && BATTLER_ALIVE(gActiveBattler) && !BATTLER_MAX_HP(gActiveBattler) && !IsHealBlocked(gActiveBattler))
				{
		...
		case ET_Aqua_Ring:
			if (gStatuses3[gActiveBattler] & STATUS3_AQUA_RING && BATTLER_ALIVE(gActiveBattler) && !BATTLER_MAX_HP(gActiveBattler) && !IsHealBlocked(gActiveBattler))
			{
		...
		case ET_Ingrain:
			if (gStatuses3[gActiveBattler] & STATUS3_ROOTED && BATTLER_ALIVE(gActiveBattler) && !BATTLER_MAX_HP(gActiveBattler) && !IsHealBlocked(gActiveBattler))
			{
				gBattleMoveDamage = GetBaseMaxHP(gActiveBattler) / 16;
				if (ITEM_EFFECT(gActiveBattler) == ITEM_EFFECT_BIG_ROOT)
		...
		case ET_Leech_Seed:
			if (gStatuses3[gActiveBattler] & STATUS3_LEECHSEED && BATTLER_ALIVE(gActiveBattler) && ABILITY(gActiveBattler) != ABILITY_MAGICGUARD && gBattleMons[gStatuses3[gActiveBattler] & STATUS3_LEECHSEED_BATTLER].hp != 0)
			{
		...
		case ET_Poison:
			if (BATTLER_ALIVE(gActiveBattler) && ABILITY(gActiveBattler) != ABILITY_MAGICGUARD)
			{
				if (gBattleMons[gActiveBattler].status1 & (STATUS_POISON | STATUS_TOXIC_POISON))
				{
					if (gBattleMons[gActiveBattler].status1 & STATUS_TOXIC_POISON && (gBattleMons[gActiveBattler].status1 & 0xF00) != 0xF00) // not 16 turns
		...
		case ET_Burn:
			if (gBattleMons[gActiveBattler].status1 & STATUS_BURN && BATTLER_ALIVE(gActiveBattler) && ABILITY(gActiveBattler) != ABILITY_MAGICGUARD)
			{
		...
		case ET_Curse:
			if (gBattleMons[gActiveBattler].status2 & STATUS2_CURSED && BATTLER_ALIVE(gActiveBattler) && ABILITY(gActiveBattler) != ABILITY_MAGICGUARD)
			{
		...
		case ET_Trap_Damage:
			if (gNewBS->brokeFreeMessage & gBitTable[gActiveBattler] && BATTLER_ALIVE(gActiveBattler))
			{
				// Broke free
			...
			else if ((gBattleMons[gActiveBattler].status2 & STATUS2_WRAPPED) && gBattleMons[gActiveBattler].hp)
			{
		...
		case ET_Octolock:
			if (gBattleMons[gActiveBattler].status2 & STATUS2_ESCAPE_PREVENTION && BATTLER_ALIVE(gActiveBattler) && gNewBS->trappedByOctolock)
			{
		...
		case ET_Magnet_Rise_Timer:
			if (gNewBS->MagnetRiseTimers[gActiveBattler] > 0 && BATTLER_ALIVE(gActiveBattler) && --gNewBS->MagnetRiseTimers[gActiveBattler] == 0)
			{
		...
		case ET_Telekinesis_Timer:
			if (gNewBS->TelekinesisTimers[gActiveBattler] > 0 && BATTLER_ALIVE(gActiveBattler) && --gNewBS->TelekinesisTimers[gActiveBattler] == 0)
			{
		...
		case ET_Heal_Block_Timer:
			if (gNewBS->HealBlockTimers[gActiveBattler] > 0 && BATTLER_ALIVE(gActiveBattler) && --gNewBS->HealBlockTimers[gActiveBattler] == 0)
			{
		...
		case ET_Embargo_Timer:
			if (gNewBS->EmbargoTimers[gActiveBattler] > 0 && BATTLER_ALIVE(gActiveBattler) && --gNewBS->EmbargoTimers[gActiveBattler] == 0)
			{
		...
		case ET_Rainbow_Timer:
			if (gBattleStruct->turnEffectsBank < 2)
			{
				if (gNewBS->RainbowTimers[gBattleStruct->turnEffectsBank] > 0 && --gNewBS->RainbowTimers[gBattleStruct->turnEffectsBank] == 0)
				{
					gBankAttacker = gBankTarget = gActiveBattler = gBattleStruct->turnEffectsBank;
		...
		case ET_Sea_Of_Fire_Timer:
			if (gBattleStruct->turnEffectsBank < 2)
			{
				if (gNewBS->SeaOfFireTimers[gBattleStruct->turnEffectsBank] > 0 && --gNewBS->SeaOfFireTimers[gBattleStruct->turnEffectsBank] == 0)
				{
					gBankAttacker = gBankTarget = gActiveBattler = gBattleStruct->turnEffectsBank;
		...
		case ET_Swamp_Timer:
			if (gBattleStruct->turnEffectsBank < 2)
			{
				if (gNewBS->SwampTimers[gBattleStruct->turnEffectsBank] > 0 && --gNewBS->SwampTimers[gBattleStruct->turnEffectsBank] == 0)
				{
					gBankAttacker = gBankTarget = gActiveBattler = gBattleStruct->turnEffectsBank;
		...
		case ET_G_Max_VineLash_Timer:
			if (gBattleStruct->turnEffectsBank < 2)
			{
				if (gNewBS->maxVineLashTimers[gBattleStruct->turnEffectsBank] > 0 && --gNewBS->maxVineLashTimers[gBattleStruct->turnEffectsBank] == 0)
				{
					gBankAttacker = gBankTarget = gActiveBattler = gBattleStruct->turnEffectsBank;
		...
		case ET_G_Max_Wildfire_Timer:
			if (gBattleStruct->turnEffectsBank < 2)
			{
				if (gNewBS->maxWildfireTimers[gBattleStruct->turnEffectsBank] > 0 && --gNewBS->maxWildfireTimers[gBattleStruct->turnEffectsBank] == 0)
				{
					gBankAttacker = gBankTarget = gActiveBattler = gBattleStruct->turnEffectsBank;
		...
		case ET_G_Max_Cannonade_Timer:
			if (gBattleStruct->turnEffectsBank < 2)
			{
				if (gNewBS->maxCannonadeTimers[gBattleStruct->turnEffectsBank] > 0 && --gNewBS->maxCannonadeTimers[gBattleStruct->turnEffectsBank] == 0)
				{
					gBankAttacker = gBankTarget = gActiveBattler = gBattleStruct->turnEffectsBank;
		...
		case ET_G_Max_Volcalith_Timer:
			if (gBattleStruct->turnEffectsBank < 2)
			{
				if (gNewBS->maxVolcalithTimers[gBattleStruct->turnEffectsBank] > 0 && --gNewBS->maxVolcalithTimers[gBattleStruct->turnEffectsBank] == 0)
				{
					gBankAttacker = gBankTarget = gActiveBattler = gBattleStruct->turnEffectsBank;
		...
		case ET_Block_B:
			...
			{
			case ET_Uproar:
				if (gBattleMons[gActiveBattler].status2 & STATUS2_UPROAR && BATTLER_ALIVE(gActiveBattler))
				{
			...
			case ET_Orbz:
				if (BATTLER_ALIVE(gActiveBattler) && gBattleMons[gActiveBattler].status1 == STATUS1_NONE)
				{
...
// Revert if back above half health
				...
				case ABILITY_POWERCONSTRUCT:
					if ((species == SPECIES_ZYGARDE || species == SPECIES_ZYGARDE_10) && gBattleMons[gActiveBattler].hp <= gBattleMons[gActiveBattler].maxHP / 2)
					{
				...
				case ABILITY_SCHOOLING:
					if (species == SPECIES_WISHIWASHI && gBattleMons[gActiveBattler].level >= 20 && gBattleMons[gActiveBattler].hp > gBattleMons[gActiveBattler].maxHP / 4)
					{
					...
					else if (species == SPECIES_WISHIWASHI_S && (gBattleMons[gActiveBattler].level < 20 || gBattleMons[gActiveBattler].hp <= gBattleMons[gActiveBattler].maxHP / 4))
					{
				...
				case ABILITY_SHIELDSDOWN:
					if (species == SPECIES_MINIOR_SHIELD && gBattleMons[gActiveBattler].hp <= gBattleMons[gActiveBattler].maxHP / 2)
					{
					...
					else if (CheckTableForSpecies(species, gMiniorCores) && gBattleMons[gActiveBattler].hp > (gBattleMons[gActiveBattler].maxHP / 2))
					{
				...
				case ABILITY_FLOWERGIFT:
					...
					case SPECIES_CHERRIM:
						if (WEATHER_HAS_EFFECT && (gBattleWeather & WEATHER_SUN_ANY) && itemEffect != ITEM_EFFECT_UTILITY_UMBRELLA)
						{
...
u32 GetLeechSeedDamage(u8 bank)
{
	...
	if (gStatuses3[bank] & STATUS3_LEECHSEED && ABILITY(bank) != ABILITY_MAGICGUARD)
...
u32 GetPoisonDamage(u8 bank)
{
	...
	if (ability != ABILITY_MAGICGUARD && ability != ABILITY_POISONHEAL)
	{
...
u32 GetBurnDamage(u8 bank)
{
	...
	if (gBattleMons[bank].status1 & STATUS_BURN && ability != ABILITY_MAGICGUARD)
	{
...
u32 GetNightmareDamage(u8 bank)
{
	...
	if (gBattleMons[bank].status2 & STATUS2_NIGHTMARE && gBattleMons[bank].status1 & STATUS_SLEEP && ABILITY(bank) != ABILITY_MAGICGUARD)
	{
...
u32 GetCurseDamage(u8 bank)
{
	...
	if (gBattleMons[bank].status2 & STATUS2_CURSED && ABILITY(bank) != ABILITY_MAGICGUARD)
	{
		damage = MathMax(1, GetBaseMaxHP(bank) / 4);
	}
...
u32 GetTrapDamage(u8 bank)
{
	...
	if (gBattleMons[bank].status2 & STATUS2_WRAPPED && ABILITY(bank) != ABILITY_MAGICGUARD)
	{
...
u32 GetSeaOfFireDamage(u8 bank)
{
	...
	if (BankSideHasSeaOfFire(bank) && !IsOfType(bank, TYPE_FIRE) && ABILITY(bank) != ABILITY_MAGICGUARD)
	{
		damage = MathMax(1, GetBaseMaxHP(bank) / 8);
	}
...
u32 GetGMaxVineLashDamage(u8 bank)
{
	...
	if (BankSideHasGMaxVineLash(bank) && !IsOfType(bank, TYPE_GRASS) && ABILITY(bank) != ABILITY_MAGICGUARD)
	{
		damage = MathMax(1, GetBaseMaxHP(bank) / 6);
	}
...
u32 GetGMaxWildfireDamage(u8 bank)
{
	...
	if (BankSideHasGMaxWildfire(bank) && !IsOfType(bank, TYPE_FIRE) && ABILITY(bank) != ABILITY_MAGICGUARD)
	{
		damage = MathMax(1, GetBaseMaxHP(bank) / 6);
	}
...
u32 GetGMaxCannonadeDamage(u8 bank)
{
	...
	if (BankSideHasGMaxCannonade(bank) && !IsOfType(bank, TYPE_WATER) && ABILITY(bank) != ABILITY_MAGICGUARD)
	{
		damage = MathMax(1, GetBaseMaxHP(bank) / 6);
	}
...
u32 GetGMaxVolcalithDamage(u8 bank)
{
	...
	if (BankSideHasGMaxVolcalith(bank) && ABILITY(bank) != ABILITY_MAGICGUARD)
	{
		damage = MathMax(1, GetBaseMaxHP(bank) / 6);
	}
	...
	do
	{
		...
		case 1:
			do
			{
				...
				if (gBattleMons[gBattleStruct->faintedActionsBank].hp == 0 && !(gBattleStruct->givenExpMons & gBitTable[gBattlerPartyIndexes[gBattleStruct->faintedActionsBank]]) && !(gAbsentBattlerFlags & gBitTable[gBattleStruct->faintedActionsBank]))
				{
		...
		case 4:
			do
			{
				...
				{
					if (IsCatchableRaidBattle() && gBattleStruct->faintedActionsBank == BANK_RAID_BOSS)
						continue; // Don't end the battle if the raid mon is KOed

					if (RAID_BATTLE_END && SIDE(gBattleStruct->faintedActionsBank) == B_SIDE_PLAYER && ViableMonCountFromBank(GetBattlerAtPosition(B_POSITION_PLAYER_LEFT)) >= 1)
					...
					{
						if (gNewBS->endTurnDone || ViableMonCountFromBank(gBattleStruct->faintedActionsBank) == 0)
						{
							...
							else
							{
								...
								for (i = 0; i < gBattlersCount; ++i)
								{
									if (gNewBS->AbsentBattlerHelper & gBitTable[i] && !(gNewBS->doSwitchInEffects & gBitTable[i]) && HITMARKER_FAINTED(i))
									{
										if (++gBattleStruct->faintedActionsBank >= gBattlersCount)
