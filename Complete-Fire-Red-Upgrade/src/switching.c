...
{
	return TryRemovePrimalWeather(bank, ability) || TryRemoveNeutralizingGas(ability) || TryRemoveUnnerve(bank) || TryActivateFlowerGift(bank);
}
...
static bool8 TryActivateFlowerGift(u8 leavingBank)
{
	...
	if (ABILITY(leavingBank) == ABILITY_AIRLOCK || ABILITY(leavingBank) == ABILITY_CLOUDNINE)
...
void atk4F_jumpifcantswitch(void)
{
	...
	if (!(T2_READ_8(gBattlescriptCurrInstr + 1) & ATK4F_DONT_CHECK_STATUSES) && !IsOfType(gActiveBattler, TYPE_GHOST) && ITEM_EFFECT(gActiveBattler) != ITEM_EFFECT_SHED_SHELL && ((gBattleMons[gActiveBattler].status2 & (STATUS2_WRAPPED | STATUS2_ESCAPE_PREVENTION)) || (gStatuses3[gActiveBattler] & STATUS3_ROOTED) || IsFairyLockActive()))
	{
	...
	else
	{
		...
		{
			if (GetMonData(&party[i], MON_DATA_HP, 0) != 0 && GetMonData(&party[i], MON_DATA_SPECIES, 0) != SPECIES_NONE && !GetMonData(&party[i], MON_DATA_IS_EGG, 0) && i != gBattlerPartyIndexes[battlerIn1] && i != gBattlerPartyIndexes[battlerIn2])
				break;
		}
...
void atk52_switchineffects(void)
{
	...
	if (!(gSideStatuses[SIDE(gActiveBattler)] & SIDE_STATUS_SPIKES) // Skip the entry hazards if there are none
		&& gNewBS->switchInEffectsState >= SwitchIn_Spikes && gNewBS->switchInEffectsState <= SwitchIn_StickyWeb)
	...
	case SwitchIn_HealingWish:
		if (gBattleMons[gActiveBattler].hp != gBattleMons[gActiveBattler].maxHP || gBattleMons[gActiveBattler].status1 != STATUS1_NONE)
		{
	...
	case SwitchIn_Spikes:
		if (CheckGrounding(gActiveBattler) && gSideTimers[SIDE(gActiveBattler)].spikesAmount > 0 && ability != ABILITY_MAGICGUARD && itemEffect != ITEM_EFFECT_HEAVY_DUTY_BOOTS)
		{
	...
	case SwitchIn_StealthRock:
		if (gSideTimers[SIDE(gActiveBattler)].srAmount > 0 && ability != ABILITY_MAGICGUARD && ability != ABILITY_MOUNTAINEER && itemEffect != ITEM_EFFECT_HEAVY_DUTY_BOOTS)
		{
	...
	case SwitchIn_Steelsurge:
		if (gSideTimers[SIDE(gActiveBattler)].steelsurge > 0 && ability != ABILITY_MAGICGUARD && itemEffect != ITEM_EFFECT_HEAVY_DUTY_BOOTS)
		{
	...
	case SwitchIn_ToxicSpikes:
		if (gSideTimers[SIDE(gActiveBattler)].tspikesAmount > 0 && CheckGrounding(gActiveBattler))
		{
	...
	case SwitchIn_StickyWeb:
		if (gSideTimers[SIDE(gActiveBattler)].stickyWeb && CheckGrounding(gActiveBattler) && itemEffect != ITEM_EFFECT_HEAVY_DUTY_BOOTS)
		{
			BattleScriptPushCursor();
	...
	case SwitchIn_EmergencyExit:
		...
		{
			if (gBattleMons[gActiveBattler].hp > 0 && gBattleMons[gActiveBattler].hp <= gBattleMons[gActiveBattler].maxHP / 2 && gBattleMons[gActiveBattler].hp + gNewBS->DamageTaken[gActiveBattler] > gBattleMons[gActiveBattler].maxHP / 2)
			{
	...
	case SwitchIn_Abilities:
		for (i = 0; i < gBattlersCount; ++i)
		{
			if (i != gActiveBattler && ABILITY(i) == ABILITY_TRACE && AbilityBattleEffects(ABILITYEFFECT_ON_SWITCHIN, i, 0, 0, 0))
				return;
		}
	...
	case SwitchIn_LastPokemonMusic:
		...
		if (gBattleTypeFlags & BATTLE_TYPE_TRAINER && !(gBattleTypeFlags & (BATTLE_TYPE_DOUBLE | BATTLE_TYPE_LINK | BATTLE_TYPE_FRONTIER | BATTLE_TYPE_TRAINER_TOWER)) && gTrainers[gTrainerBattleOpponent_A].trainerClass == CLASS_LEADER && SIDE(gActiveBattler) == B_SIDE_OPPONENT // So player accepting offer to switching out doesn't trigger this yet
...
void atk8F_forcerandomswitch(void)
{
	...
	if (gBattleTypeFlags & (BATTLE_TYPE_TRAINER | BATTLE_TYPE_DOUBLE)) // Trainer Battle or Wild Double Battle
	{
		...
		if ((gBattleTypeFlags & BATTLE_TYPE_LINK && gBattleTypeFlags & BATTLE_TYPE_MULTI) || (gBattleTypeFlags & BATTLE_TYPE_LINK && gBattleTypeFlags & BATTLE_TYPE_FRONTIER && SIDE(bankDef) == B_SIDE_PLAYER) || (gBattleTypeFlags & BATTLE_TYPE_TWO_OPPONENTS && SIDE(bankDef) == B_SIDE_OPPONENT) || (gBattleTypeFlags & BATTLE_TYPE_INGAME_PARTNER && SIDE(bankDef) == B_SIDE_PLAYER))
		{
			monsCount = 3;
		...
		for (i = firstMonId; i < lastMonId; ++i)
		{
			if (i != battler1PartyId && i != battler2PartyId && MON_CAN_BATTLE(&party[i]))
				validMons++;
		}
		...
		else
		{
			if (TryDoForceSwitchOut())
			{
				do
				{
					i = Random() % monsCount;
					i += firstMonId;
				} while (i == battler1PartyId || i == battler2PartyId || !MON_CAN_BATTLE(&party[i]));
			}
...
bool8 HasNoMonsToSwitch(u8 battler)
{
	...
	for (i = firstMonId; i < lastMonId; ++i)
	{
		if (GetMonData(&party[i], MON_DATA_HP, 0) != 0 && GetMonData(&party[i], MON_DATA_SPECIES, 0) != SPECIES_NONE && GetMonData(&party[i], MON_DATA_SPECIES2, 0) != SPECIES_EGG && !GetMonData(&party[i], MON_DATA_IS_EGG, 0) && i != gBattlerPartyIndexes[battlerIn1] && i != gBattlerPartyIndexes[battlerIn2])
			break;
	}
...
void PartyMenuSwitchingUpdate(void)
{
	...
	if ((gBattleMons[gActiveBattler].status2 & (STATUS2_WRAPPED | STATUS2_ESCAPE_PREVENTION)) || (gStatuses3[gActiveBattler] & (STATUS3_ROOTED | STATUS3_SKY_DROP_TARGET)) || IsFairyLockActive())
	{
		EmitChoosePokemon(0, PARTY_CANT_SWITCH, 6, ABILITY_NONE, gBattleStruct->field_60[gActiveBattler]);
	}
	else if (((i = ABILITY_ON_OPPOSING_FIELD(gActiveBattler, ABILITY_SHADOWTAG)) && ABILITY(gActiveBattler) != ABILITY_SHADOWTAG) || ((i = ABILITY_ON_OPPOSING_FIELD(gActiveBattler, ABILITY_ARENATRAP)) && CheckGrounding(gActiveBattler)) || ((i = AbilityBattleEffects(ABILITYEFFECT_CHECK_FIELD_EXCEPT_BANK, gActiveBattler, ABILITY_MAGNETPULL, 0, 0)) && IsOfType(gActiveBattler, TYPE_STEEL)))
	{
...
bool8 WillFaintFromEntryHazards(struct Pokemon *mon, u8 side)
{
	...
	if (gSideStatuses[side] & SIDE_STATUS_SPIKES && GetMonAbility(mon) != ABILITY_MAGICGUARD && ItemId_GetHoldEffect(GetMonData(mon, MON_DATA_HELD_ITEM, NULL)) != ITEM_EFFECT_HEAVY_DUTY_BOOTS)
	{
		if (gSideTimers[side].srAmount > 0 && GetMonAbility(GetBankPartyData(bank)) != ABILITY_MOUNTAINEER)
