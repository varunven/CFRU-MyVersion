...
void BuildTrainerPartySetup(void)
{
	...
	if (gBattleTypeFlags & BATTLE_TYPE_INGAME_PARTNER && (ViableMonCount(gEnemyParty) > 1 || IsRaidBattle()))
	{
		...
		{
			if (IsRandomBattleTowerBattle() || GetMonData(&gPlayerParty[3], MON_DATA_SPECIES, NULL) == SPECIES_NONE) // Ideally the partner's Pokemon should be prepared beforehand. This is a backup measure
...
u16 GiveRandomFrontierMonByTier(u8 side, u8 tier, u16 spreadType)
{
	...
	} while (IsPokemonBannedBasedOnStreak(spread->species, spread->item, NULL, 0, 0, tier, TRUE) || PokemonTierBan(spread->species, spread->item, spread, NULL, tier, CHECK_BATTLE_TOWER_SPREADS));
...
static u8 CreateNPCTrainerParty(struct Pokemon *const party, const u16 trainerId, const bool8 firstTrainer, const bool8 side)
{
	...
	if (((gBattleTypeFlags & (BATTLE_TYPE_TRAINER | BATTLE_TYPE_EREADER_TRAINER | BATTLE_TYPE_TRAINER_TOWER)) == BATTLE_TYPE_TRAINER) || (gBattleTypeFlags & BATTLE_TYPE_INGAME_PARTNER))
	{
...
// Choose Trainer IVs
		...
		{
#ifdef OPEN_WORLD_TRAINERS
			if ((firstTrainer && gTrainerBattleOpponent_A < DYNAMIC_TRAINER_LIMIT) || (!firstTrainer && VarGet(VAR_SECOND_OPPONENT) < DYNAMIC_TRAINER_LIMIT))
			{
...
// Give EVs
			...
			if (gTrainers[trainerId].partyFlags == (PARTY_FLAG_CUSTOM_MOVES | PARTY_FLAG_HAS_ITEM) && trainer->aiFlags > 1
#ifdef VAR_GAME_DIFFICULTY
				&& gameDifficulty != OPTIONS_EASY_DIFFICULTY
#endif
				&& spreadNum != 0 && spreadNum < NELEMS(gTrainersWithEvsSpreads)) // Valid id
			{
...
static bool8 CanTrainerEvolveMon(void)
{
	return sTrainerBattleMode == TRAINER_BATTLE_SINGLE_SCALED || sTrainerBattleMode == TRAINER_BATTLE_DOUBLE_SCALED || sTrainerBattleMode == TRAINER_BATTLE_SINGLE_NO_INTRO_TEXT_SCALED
#ifdef UNBOUND
		   || sTrainerBattleMode == TRAINER_BATTLE_REMATCH || sTrainerBattleMode == TRAINER_BATTLE_REMATCH_DOUBLE || sTrainerBattleMode == TRAINER_BATTLE_REMATCH_TWO_OPPONENTS
#endif
		;
}
...
static u8 BuildFrontierParty(struct Pokemon *const party, const u16 trainerId, const u8 tier, const bool8 firstTrainer, const bool8 forPlayer, const u8 side)
{
	...
	for (i = 0; i < monsCount; ++i)
	{
		...
		do
		{
			...
			itemEffect = ((gMain.inBattle && gBattleTypeFlags & BATTLE_TYPE_BATTLE_CIRCUS && gBattleCircusFlags & BATTLE_CIRCUS_MAGIC_ROOM)) ? 0 : ItemId_GetHoldEffect(item);
			...
			if (!IsPokemonBannedBasedOnStreak(species, item, builder->speciesArray, monsCount, trainerId, tier, forPlayer) && (!builder->speciesOnTeam[dexNum] || tier == BATTLE_FACILITY_NO_RESTRICTIONS) && (!ItemAlreadyOnTeam(item, monsCount, builder->itemArray) || tier == BATTLE_FACILITY_NO_RESTRICTIONS) && (tier == BATTLE_FACILITY_MEGA_BRAWL || itemEffect != ITEM_EFFECT_MEGA_STONE || item == ITEM_ULTRANECROZIUM_Z || !builder->itemEffectOnTeam[ITEM_EFFECT_MEGA_STONE]) && ((itemEffect != ITEM_EFFECT_Z_CRYSTAL && item != ITEM_ULTRANECROZIUM_Z) || !builder->itemEffectOnTeam[ITEM_EFFECT_Z_CRYSTAL]) && !PokemonTierBan(species, item, spread, NULL, tier, CHECK_BATTLE_TOWER_SPREADS) && !(tier == BATTLE_FACILITY_MONOTYPE && TeamNotAllSameType(species, item, monsCount, builder->speciesArray, builder->itemArray)) && !(tier == BATTLE_FACILITY_GS_CUP && !IsFrontierSingles(battleType) && TooManyLegendariesOnGSCupTeam(species, monsCount, builder->speciesArray)) && !((trainerId == BATTLE_TOWER_TID || forPlayer || (trainerId == BATTLE_FACILITY_MULTI_TRAINER_TID && IsRandomBattleTowerBattle())) && TeamDoesntHaveSynergy(spread, builder)))
			{
...
static void SetWildMonHeldItem(void)
{
	...
	if (!GetMonData(&gPlayerParty[0], MON_DATA_IS_EGG, 0) && (GetMonAbility(&gPlayerParty[0]) == ABILITY_COMPOUNDEYES || GetMonAbility(&gPlayerParty[0]) == ABILITY_SUPERLUCK))
	{
		var1 = 20;
		var2 = 80;
	}
...
void GiveMonNatureAndAbility(struct Pokemon *mon, u8 nature, u8 abilityNum, bool8 forceShiny, bool8 keepGender, bool8 keepLetterCore)
{
	...
	} while (GetNatureFromPersonality(personality) != nature || (keepGender && GetGenderFromSpeciesAndPersonality(species, personality) != gender) || (keepLetterCore && species == SPECIES_UNOWN && GetUnownLetterFromPersonality(personality) != letter) // Make sure the Unown letter doesn't change
...
static void AdjustTypesForMegas(const u16 species, const u16 item, u8 *const type1, u8 *const type2)
{
	...
	if (megaSpecies != SPECIES_NONE)
	{
		...
		if (*type1 != megaType1 && *type1 != megaType2)
			*type1 = TYPE_BLANK;

		if (*type2 != megaType1 && *type2 != megaType2)
			*type2 = TYPE_BLANK;
	}
}
...
static bool8 TeamNotAllSameType(const u16 species, const u16 item, const u8 partySize, const species_t *const speciesArray, const item_t *const itemArray)
{
	...
	for (int i = 0; i < partySize; ++i)
	{
		...
		else
		{
			...
			{
				if (typeOnTeam[j] && j != checkType1 // First Pokemon had type that second or third etc. did not,
					&& j != checkType2)				 // then type no longer on team
					typeOnTeam[j] = FALSE;
			}
		}

		if ((type1 != checkType1 || type1 == TYPE_BLANK || checkType1 == TYPE_BLANK) && (type1 != checkType2 || type1 == TYPE_BLANK || checkType2 == TYPE_BLANK) && (type2 != checkType1 || type2 == TYPE_BLANK || checkType1 == TYPE_BLANK) && (type2 != checkType2 || type2 == TYPE_BLANK || checkType2 == TYPE_BLANK))
			return TRUE;
	}
...
static bool8 PokemonTierBan(const u16 species, const u16 item, const struct BattleTowerSpread *const spread, const struct Pokemon *const mon, const u8 tier, const u8 checkFromLocationType)
{
	...
	case BATTLE_FACILITY_MEGA_BRAWL:
	case BATTLE_FACILITY_DYNAMAX_STANDARD:
		if (CheckTableForSpecies(species, gBattleTowerStandardSpeciesBanList) || CheckTableForItem(item, gBattleTowerStandard_ItemBanList))
			return TRUE;
		break;
	...
	STANDARD_OU_CHECK:
		...
		if (IsFrontierSingles(battleFormat))
		{
			...
			{
				if (CheckTableForSpecies(species, gSmogonNationalDexOU_SpeciesBanList) || CheckTableForItem(item, gSmogonNationalDexOU_ItemBanList))
					return TRUE;
			}
			else // Gen 7 OU
			{
				if (CheckTableForSpecies(species, gSmogonOU_SpeciesBanList) || CheckTableForItem(item, gSmogonOU_ItemBanList))
					return TRUE;
			}
			...
			{
				if (CheckTableForMove(moveLoc[i], gSmogon_MoveBanList) || moveLoc[i] == MOVE_BATONPASS)
					return TRUE;
			}
		}
		else // Double Battles
		{
			...
			if (CheckTableForSpecies(species, gSmogonOUDoubles_SpeciesBanList) || CheckTableForItem(item, gSmogonOUDoubles_ItemBanList))
				return TRUE;
			...
			for (i = 0; i < MAX_MON_MOVES; ++i)
			{
				if (CheckTableForMove(moveLoc[i], gSmogon_MoveBanList) || CheckTableForMove(moveLoc[i], gSmogonOUDoubles_MoveBanList))
					return TRUE;
				...
				{
					if (knowsHealingMove || knowsPainSplit || knowsFling)
						return TRUE;
				}
			}
		}
	...
	case BATTLE_FACILITY_LITTLE_CUP:
	case BATTLE_FACILITY_LC_CAMOMONS:
		if (!CheckTableForSpecies(species, gSmogonLittleCup_SpeciesList) || CheckTableForItem(item, gSmogonLittleCup_ItemBanList))
			return TRUE; // Banned
	...
	case BATTLE_FACILITY_MC_CAMOMONS:
		...
		{
			if (!CheckTableForSpecies(species, gMiddleCup_SpeciesList) || CheckTableForItem(item, gMiddleCup_ItemBanList))
				return TRUE; // Banned
		...
		else // GS Cup
		{
			...
			if (CheckTableForSpecies(species, gBattleTowerStandardSpeciesBanList) && !CheckTableForSpecies(species, gGSCup_LegendarySpeciesList))
				return TRUE;
		}
		break;
	...
	case BATTLE_FACILITY_MONOTYPE:
		...
		if (CheckTableForSpecies(species, gSmogonMonotype_SpeciesBanList) || CheckTableForItem(item, gSmogonMonotype_ItemBanList))
			return TRUE;
		...
		for (i = 0; i < MAX_MON_MOVES; ++i)
		{
			if (CheckTableForMove(moveLoc[i], gSmogon_MoveBanList) || moveLoc[i] == MOVE_BATONPASS || moveLoc[i] == MOVE_SWAGGER)
				return TRUE;
		}
		break;
	...
	case BATTLE_FACILITY_SCALEMONS:
		if (CheckTableForSpecies(species, gSmogonScalemons_SpeciesBanList) || CheckTableForItem(item, gSmogonScalemons_ItemBanList))
			return TRUE;
	...
	case BATTLE_FACILITY_350_CUP:
		if (CheckTableForSpecies(species, gSmogon350Cup_SpeciesBanList) || CheckTableForItem(item, gSmogon350Cup_ItemBanList))
			return TRUE;
	...
	case BATTLE_FACILITY_AVERAGE_MONS:
		if (CheckTableForSpecies(species, gSmogonAverageMons_SpeciesBanList) || CheckTableForItem(item, gSmogonAverageMons_ItemBanList))
			return TRUE;
...
static bool8 IsPokemonBannedBasedOnStreak(u16 species, u16 item, u16 *speciesArray, u8 monsCount, u16 trainerId, u8 tier, bool8 forPlayer)
{
	...
	if (!forPlayer && trainerId == BATTLE_TOWER_TID && (tier == BATTLE_FACILITY_STANDARD || tier == BATTLE_FACILITY_DYNAMAX_STANDARD))
	{
		if (megasZMovesBannedInTier && (IsZCrystal(item) || IsMegaStone(item))) // Don't give the AI Pokemon with bad items
			return TRUE;

		// Battles get more difficult the higher the streak.
		if (streak < 10)
		{
			if (IsMegaStone(item) || IsZCrystal(item) || GetBaseStatsTotal(species) >= 520)
				return TRUE;
		}
		else if (streak < 20)
		{
			if (IsMegaStone(item) || GetBaseStatsTotal(species) >= 540)
				return TRUE;
		}
		else if (streak < 30)
		{
		...
		{
			if (GetBaseStatsTotal(species) >= 570 && BaseStatsTotalGEAlreadyOnTeam(570, monsCount, speciesArray))
				return TRUE;
		}
	}
	else if ((forPlayer || trainerId == BATTLE_FACILITY_MULTI_TRAINER_TID) && (tier == BATTLE_FACILITY_STANDARD || tier == BATTLE_FACILITY_DYNAMAX_STANDARD))
	{
		streak = GetMaxBattleTowerStreakForTier(tier);

		if (megasZMovesBannedInTier && (IsZCrystal(item) || IsMegaStone(item))) // Don't give the player Pokemon with bad items
			return TRUE;

		// Better Pokemon are given to the player the better the streak
		if (streak < 10)
		{
			if (IsMegaStone(item) || IsZCrystal(item) || GetBaseStatsTotal(species) >= 540)
				return TRUE;
		}
		else if (streak < 20)
		{
			if (IsMegaStone(item) || GetBaseStatsTotal(species) >= 550)
				return TRUE;
		}
		else if (streak < 30)
		{
	...
	else if (trainerId == BATTLE_TOWER_SPECIAL_TID)
	{
		...
		if (streak < 20 && (tier == BATTLE_FACILITY_STANDARD || tier == BATTLE_FACILITY_DYNAMAX_STANDARD))
		{
...
static bool8 TeamDoesntHaveSynergy(const struct BattleTowerSpread *const spread, const struct TeamBuilder *const builder)
{
	...
	u8 itemEffect = ((gMain.inBattle && gBattleTypeFlags & BATTLE_TYPE_BATTLE_CIRCUS && gBattleCircusFlags & BATTLE_CIRCUS_MAGIC_ROOM)) ? 0 : ItemId_GetHoldEffect(spread->item);
	...
	if (builder->monsCount <= 4)
	{
		if (IS_SINGLE_BATTLE)
		{
			...
			{
				if (builder->numStalls >= 1 || builder->partyIndex[CLERIC] != 0xFF)
			...
			{
				if (builder->partyIndex[BATON_PASSER] != 0xFF || builder->partyIndex[HAZARDS_SETUP] != 0xFF || builder->partyIndex[SCREENER] != 0xFF)
	...
	else // 5-6 Pokemon
	{
		if (IS_SINGLE_BATTLE)
		{
			...
			{
				if (builder->tier == BATTLE_FACILITY_MONOTYPE || (IsMiddleCupTier(builder->tier) && IsFrontierSingles(builder->battleType)))
				{
					// Let more stally Pokemon in Middle Cup and Monotype
					if (builder->numStalls >= 4 || (builder->numStalls == 3 && builder->partyIndex[CLERIC] != 0xFF))
						return TRUE; // Max two stall/one stall + one cleric Pokemon per 6v6 Team
				}
				else
				{
					if (builder->numStalls >= 2 || (builder->numStalls == 1 && builder->partyIndex[CLERIC] != 0xFF))
						return TRUE; // Max two stall/one stall + one cleric Pokemon per 6v6 Team
				}
			}
			else if (IsClassCleric(class))
			{
				if (builder->partyIndex[CLERIC] != 0xFF || builder->numStalls >= 2)
		...
		for (i = 0; i < PARTY_SIZE; ++i)
		{
			if (builder->spreads[i]->modifyMovesDoubles)
			{
				...
				for (j = 0; j < ARRAY_COUNT(sDoubleSpreadReplacementMoves); ++j)
				{
					...
					{
						if (sDoubleSpreadReplacementMoves[j].noIfImmunity != 0 && builder->partyIndex[sDoubleSpreadReplacementMoves[j].noIfImmunity] != i)
							continue;

						if (sDoubleSpreadReplacementMoves[j].yesIfImmunity == 0 || builder->partyIndex[sDoubleSpreadReplacementMoves[j].yesIfImmunity] != i) // There's an immunity on some other Pokemon
						{
...
#define INDEX_CHECK(partyId) (index < 2 && partyId > index && partyId != 0xFF)
		...
		if (i == ARRAY_COUNT(sImmunities)) // The two Pokemon up front aren't meant to work off of each other
		{
			...
			if (INDEX_CHECK(defiantIndex) && party == gEnemyParty && (GetMonAbility(&gPlayerParty[0]) == ABILITY_INTIMIDATE || GetMonAbility(&gPlayerParty[1]) == ABILITY_INTIMIDATE))
...
static void TryShuffleMovesForCamomons(struct Pokemon *party, u8 tier, u16 trainerId)
{
	...
	if (IsCamomonsTier(tier) && trainerId != FRONTIER_BRAIN_TID)
	{
		for (i = 0; i < PARTY_SIZE; ++i)
		{
			if (GetMonData(&party[i], MON_DATA_SPECIES, NULL) == SPECIES_NONE || GetMonData(&party[i], MON_DATA_HP, NULL) == 0)
				continue;
...
bool8 IsMonAllowedInBattleTower(struct Pokemon *mon)
{
	if (FlagGet(FLAG_BATTLE_FACILITY))
	{
		...
		if (tier != BATTLE_FACILITY_NO_RESTRICTIONS) // Free for all has no duplicate species or item restriction
		{
			...
			if (SpeciesAlreadyOnTeam(mon->species, partySize, speciesArray) || (DuplicateItemsAreBannedInTier(tier, VarGet(VAR_BATTLE_FACILITY_BATTLE_TYPE)) && ItemAlreadyOnTeam(mon->item, partySize, itemArray)) || (tier == BATTLE_FACILITY_MONOTYPE && TeamNotAllSameType(mon->species, mon->item, partySize, speciesArray, itemArray)) || (tier == BATTLE_FACILITY_GS_CUP && !IsFrontierSingles(VarGet(VAR_BATTLE_FACILITY_BATTLE_TYPE)) && TooManyLegendariesOnGSCupTeam(mon->species, partySize, speciesArray)))
				return FALSE;
		}
	}
