...
bool8 CheckGrounding(u8 bank)
{
	...
	if (IsGravityActive() || GetBankItemEffect(bank) == ITEM_EFFECT_IRON_BALL || (gStatuses3[bank] & (STATUS3_SMACKED_DOWN | STATUS3_ROOTED)))
		return GROUNDED;

	else if ((gStatuses3[bank] & (STATUS3_LEVITATING | STATUS3_TELEKINESIS | STATUS3_IN_AIR)) || ITEM_EFFECT(bank) == ITEM_EFFECT_AIR_BALLOON || ABILITY(bank) == ABILITY_LEVITATE || gBattleMons[bank].type3 == TYPE_FLYING || gBattleMons[bank].type1 == TYPE_FLYING || gBattleMons[bank].type2 == TYPE_FLYING)
		return IN_AIR;
...
bool8 NonInvasiveCheckGrounding(u8 bank)
{
	...
	if (IsGravityActive() || GetRecordedItemEffect(bank) == ITEM_EFFECT_IRON_BALL || (gStatuses3[bank] & (STATUS3_SMACKED_DOWN | STATUS3_ROOTED)))
		return GROUNDED;

	else if ((gStatuses3[bank] & (STATUS3_LEVITATING | STATUS3_TELEKINESIS | STATUS3_IN_AIR)) || GetRecordedItemEffect(bank) == ITEM_EFFECT_AIR_BALLOON || GetRecordedAbility(bank) == ABILITY_LEVITATE || gBattleMons[bank].type3 == TYPE_FLYING || gBattleMons[bank].type1 == TYPE_FLYING || gBattleMons[bank].type2 == TYPE_FLYING)
		return IN_AIR;
...
u8 ViableMonCountFromBankLoadPartyRange(u8 bank)
{
	...
	{
		if (party[i].species != SPECIES_NONE && !GetMonData(&party[i], MON_DATA_IS_EGG, NULL) && party[i].hp != 0)
			++count;
	}
...
{
	if (!(gBattleMoves[move].flags & FLAG_MAKES_CONTACT) || ITEM_EFFECT(bank) == ITEM_EFFECT_PROTECTIVE_PADS || ABILITY(bank) == ABILITY_LONGREACH)
		return FALSE;
...
{
	if (!(gBattleMoves[move].flags & FLAG_MAKES_CONTACT) || GetMonItemEffect(mon) == ITEM_EFFECT_PROTECTIVE_PADS || GetMonAbility(mon) == ABILITY_LONGREACH)
		return FALSE;
...
{
	if ((gProtectStructs[bank].protected && !IsDynamaxed(bank)) || gProtectStructs[bank].KingsShield || gProtectStructs[bank].SpikyShield || gProtectStructs[bank].BanefulBunker || gProtectStructs[bank].obstruct || gSideStatuses[SIDE(bank)] & (SIDE_STATUS_CRAFTY_SHIELD | SIDE_STATUS_MAT_BLOCK | SIDE_STATUS_QUICK_GUARD | SIDE_STATUS_WIDE_GUARD))
	{
		if (!IsDynamaxed(bank))
...
{
	if (gProtectStructs[bankDef].protected || gProtectStructs[bankDef].SpikyShield || gProtectStructs[bankDef].BanefulBunker || gProtectStructs[bankDef].obstruct)
	{
		return TRUE;
	}
	else if ((gProtectStructs[bankDef].KingsShield || (gSideStatuses[SIDE(bankDef)] & SIDE_STATUS_MAT_BLOCK)) && SPLIT(move) != SPLIT_STATUS)
	{
		return TRUE;
	}
	...
	else if (gSideStatuses[SIDE(bankDef)] & (SIDE_STATUS_WIDE_GUARD) && (gBattleMoves[move].target == MOVE_TARGET_BOTH || gBattleMoves[move].target == MOVE_TARGET_FOES_AND_ALLY))
	{
		return TRUE;
	}
...
bool8 IsUnusableMove(u16 move, u8 bank, u8 check, u8 pp, u8 ability, u8 holdEffect, u16 choicedMove)
{
	...
	if (move == MOVE_NONE && check & MOVE_LIMITATION_ZEROMOVE)
		...
		return TRUE;
	else if (!isMaxMove && (holdEffect == ITEM_EFFECT_CHOICE_BAND || ability == ABILITY_GORILLATACTICS) && choicedMove != 0 && choicedMove != 0xFFFF && choicedMove != move && check & MOVE_LIMITATION_CHOICE)
	{
		return TRUE;
	}
...
{
	if (SIDE(bankAtk) == SIDE(gSideTimers[defSide].followmeTarget) || IsMoveRedirectionPrevented(move, ABILITY(bankAtk)) || gSideTimers[defSide].followmeTimer == 0 || !BATTLER_ALIVE(gSideTimers[defSide].followmeTarget) || (gNewBS->ragePowdered & gBitTable[defSide] && !IsAffectedByPowder(bankAtk)))
		return FALSE;
...
{
	return move == MOVE_SKYDROP || move == MOVE_SNIPESHOT
...
{
	if (!BATTLER_ALIVE(bank) || bank >= gBattlersCount || gAbsentBattlerFlags & gBitTable[bank])
	{
		return FALSE;
	}
...
u8 GetIllusionPartyNumber(u8 bank)
{
	...
	if (gStatuses3[bank] & STATUS3_ILLUSION)
	{
		...
		for (u32 i = lastMonId - 1; i >= firstMonId; --i) // Loop through party in reverse order
		{
			...
			if (party[i].species == SPECIES_NONE || party[i].hp == 0 || GetMonData(&party[i], MON_DATA_IS_EGG, NULL))
				continue;
...
bool8 IsFirstAttacker(u8 bank)
{
	...
	{
		if (gActionsByTurnOrder[i] == ACTION_USE_ITEM || gActionsByTurnOrder[i] == ACTION_SWITCH)
			continue;
...
bool8 SymbiosisCanActivate(u8 giverBank, u8 receiverBank)
{
	...
	if (!(gBattleTypeFlags & BATTLE_TYPE_DOUBLE) || ABILITY(giverBank) != ABILITY_SYMBIOSIS || ITEM(receiverBank) != ITEM_NONE || ITEM(giverBank) == ITEM_NONE || !CanTransferItem(gBattleMons[giverBank].species, item) || !CanTransferItem(gBattleMons[receiverBank].species, item))
		return FALSE;
...
{
	return ability != ABILITY_OVERCOAT && itemEffect != ITEM_EFFECT_SAFETY_GOGGLES && type1 != TYPE_GRASS && type2 != TYPE_GRASS && type3 != TYPE_GRASS;
}

bool8 MoveIgnoresSubstitutes(u16 move, u8 atkAbility)
{
	return CheckSoundMove(move) || (atkAbility == ABILITY_INFILTRATOR && move != MOVE_TRANSFORM && move != MOVE_SKYDROP) || CheckTableForMove(move, gSubstituteBypassMoves);
}
...
{
	if (SPLIT(move) != SPLIT_STATUS && !IsAnyMaxMove(move) && !CheckTableForMove(move, gParentalBondBannedMoves) && !IsTwoTurnsMove(move) && gBattleMoves[move].effect != EFFECT_0HKO && gBattleMoves[move].effect != EFFECT_MULTI_HIT && gBattleMoves[move].effect != EFFECT_TRIPLE_KICK && gBattleMoves[move].effect != EFFECT_DOUBLE_HIT)
	{
		if (IS_DOUBLE_BATTLE)
		{
...
{
	if (CheckTableForMove(move, gMovesThatChangePhysicality) && SPLIT(move) != SPLIT_STATUS)
	{
...
u8 AttacksThisTurn(u8 bank, u16 move) // Note: returns 1 if it's a charging turn, otherwise 2
{
	...
	if (moveEffect == EFFECT_SKULL_BASH || moveEffect == EFFECT_RAZOR_WIND || moveEffect == EFFECT_SKY_ATTACK || moveEffect == EFFECT_SOLARBEAM || moveEffect == EFFECT_SEMI_INVULNERABLE || moveEffect == EFFECT_BIDE || move == MOVE_GEOMANCY || move == MOVE_SKYDROP)
	{
...
bool8 DoesSleepClausePrevent(u8 bank)
{
	if (gBattleTypeFlags & BATTLE_TYPE_FRONTIER)
	{
		...
		case BATTLE_FACILITY_NATIONAL_DEX_OU:;
			...
			{
				if (GetMonData(&party[i], MON_DATA_HP, NULL) != 0 && !GetMonData(&party[i], MON_DATA_IS_EGG, NULL) && GetMonData(&party[i], MON_DATA_STATUS, NULL) & STATUS_SLEEP) // Someone on team is already asleep
					return TRUE;
			}
		}
	}
...
bool8 CanBeGeneralStatused(u8 bank, bool8 checkFlowerVeil)
{
	...
	if (ABILITY(bank) == ABILITY_SHIELDSDOWN && GetBankPartyData(bank)->species == SPECIES_MINIOR_SHIELD) // Prevents Ditto from getting this benefit
		return FALSE;
#endif
...
bool8 CanBeInfatuated(u8 bankDef, u8 bankAtk)
{
	...
	return BATTLER_ALIVE(bankDef) && !(gBattleMons[bankDef].status2 & STATUS2_INFATUATION) && ABILITY(bankDef) != ABILITY_OBLIVIOUS && GetGenderFromSpeciesAndPersonality(speciesAttacker, personalityAttacker) != GetGenderFromSpeciesAndPersonality(speciesTarget, personalityTarget) && GetGenderFromSpeciesAndPersonality(speciesAttacker, personalityAttacker) != MON_GENDERLESS && GetGenderFromSpeciesAndPersonality(speciesTarget, personalityTarget) != MON_GENDERLESS && !AbilityBattleEffects(ABILITYEFFECT_CHECK_BANK_SIDE, bankDef, ABILITY_AROMAVEIL, 0, 0);
}

bool8 IsTrickRoomActive(void)
{
	return gNewBS->TrickRoomTimer > 0 || (IS_BATTLE_CIRCUS && gBattleCircusFlags & BATTLE_CIRCUS_TRICK_ROOM);
}

bool8 IsMagicRoomActive(void)
{
	return gNewBS->MagicRoomTimer > 0 || (IS_BATTLE_CIRCUS && gBattleCircusFlags & BATTLE_CIRCUS_MAGIC_ROOM);
}

bool8 IsWonderRoomActive(void)
{
	return gNewBS->WonderRoomTimer > 0 || (IS_BATTLE_CIRCUS && gBattleCircusFlags & BATTLE_CIRCUS_WONDER_ROOM);
}

bool8 IsGravityActive(void)
{
	return gNewBS->GravityTimer > 0 || (IS_BATTLE_CIRCUS && gBattleCircusFlags & BATTLE_CIRCUS_GRAVITY);
}

bool8 IsIonDelugeActive(void)
{
	return gNewBS->IonDelugeTimer > 0 || (IS_BATTLE_CIRCUS && gBattleCircusFlags & BATTLE_CIRCUS_ION_DELUGE);
}

bool8 IsFairyLockActive(void)
{
	return gNewBS->FairyLockTimer > 0 || (IS_BATTLE_CIRCUS && gBattleCircusFlags & BATTLE_CIRCUS_FAIRY_LOCK);
}

bool8 IsMudSportActive(void)
{
	return gNewBS->MudSportTimer > 0 || (IS_BATTLE_CIRCUS && gBattleCircusFlags & BATTLE_CIRCUS_MUD_SPORT);
}

bool8 IsWaterSportActive(void)
{
	return gNewBS->WaterSportTimer > 0 || (IS_BATTLE_CIRCUS && gBattleCircusFlags & BATTLE_CIRCUS_WATER_SPORT);
}
...
{
	return gSideStatuses[SIDE(bank)] & SIDE_STATUS_SAFEGUARD || (IS_BATTLE_CIRCUS && gBattleCircusFlags & BATTLE_CIRCUS_SAFEGUARD);
}

bool8 BankSideHasMist(u8 bank)
{
	return gSideStatuses[SIDE(bank)] & SIDE_STATUS_MIST || (IS_BATTLE_CIRCUS && gBattleCircusFlags & BATTLE_CIRCUS_MIST);
}

bool8 BankSideHasSeaOfFire(u8 bank)
{
	return gNewBS->SeaOfFireTimers[SIDE(bank)] || (IS_BATTLE_CIRCUS && gBattleCircusFlags & BATTLE_CIRCUS_SEA_OF_FIRE);
}

bool8 BankSideHasRainbow(u8 bank)
{
	return gNewBS->RainbowTimers[SIDE(bank)] || (IS_BATTLE_CIRCUS && gBattleCircusFlags & BATTLE_CIRCUS_RAINBOW);
}
...
{
	return (gBattleMons[bank].status2 & STATUS2_CONFUSION) != 0 || (IS_BATTLE_CIRCUS && gBattleCircusFlags & BATTLE_CIRCUS_CONFUSED && ABILITY(bank) != ABILITY_OWNTEMPO);
}

bool8 IsTaunted(u8 bank)
{
	return gDisableStructs[bank].tauntTimer > 0 || (IS_BATTLE_CIRCUS && gBattleCircusFlags & BATTLE_CIRCUS_TAUNT && ABILITY(bank) != ABILITY_OBLIVIOUS);
}

bool8 IsTormented(u8 bank)
{
	return (gBattleMons[bank].status2 & STATUS2_TORMENT) != 0 || (IS_BATTLE_CIRCUS && gBattleCircusFlags & BATTLE_CIRCUS_TORMENT);
}

bool8 IsHealBlocked(u8 bank)
{
	return gNewBS->HealBlockTimers[bank] > 0 || (IS_BATTLE_CIRCUS && gBattleCircusFlags & BATTLE_CIRCUS_HEAL_BLOCK);
}

bool8 CantUseSoundMoves(u8 bank)
{
	return gNewBS->ThroatChopTimers[bank] > 0 || (IS_BATTLE_CIRCUS && gBattleCircusFlags & BATTLE_CIRCUS_THROAT_CHOP);
}

bool8 IsLaserFocused(u8 bank)
{
	return gNewBS->LaserFocusTimers[bank] > 0 || (IS_BATTLE_CIRCUS && gBattleCircusFlags & BATTLE_CIRCUS_ALWAYS_CRIT);
}

bool8 IsAbilitySuppressed(u8 bank)
{
	return (gStatuses3[bank] & STATUS3_ABILITY_SUPPRESS) != 0 || (IS_BATTLE_CIRCUS && gBattleCircusFlags & BATTLE_CIRCUS_ABILITY_SUPPRESSION);
}
...
bool8 CantScoreACrit(u8 bank, struct Pokemon *mon)
{
	...
	return (gStatuses3[bank] & STATUS3_CANT_SCORE_A_CRIT) != 0 || (IS_BATTLE_CIRCUS && gBattleCircusFlags & BATTLE_CIRCUS_NO_CRITS);
}
